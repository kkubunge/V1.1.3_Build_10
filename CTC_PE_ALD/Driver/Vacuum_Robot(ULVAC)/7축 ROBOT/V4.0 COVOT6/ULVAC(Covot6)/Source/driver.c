...
//-----------------------------------------------------------------------------------
void TimeLog_Axis( char* str , int Mode ) {
	FILE *fp = NULL;
	char Path[MAX_PATH], DirectoryName[MAX_PATH] , FileName[64];
	SYSTEMTIME st;
	...
	{
		//     Ti A D To B  C Z
		sprintf( FileName , "%04d%02d%02d_AxisLoad.log" , st.wYear , st.wMonth , st.wDay );
		if ( strlen(LogDrive) >= 2 ) {//Log드라이브 선택에 따라 다른 경로로 분기되도록 수정(2010.01.25, ybkim)
			sprintf( Path , "DATALOG\\VAC%d\\%s" , Robot_Pos, FileName );
		}
		else if ( strlen(LogDrive) == 1 ) {
	...
	}else if ( Mode == AXIS_CNT )
	{
		//  N: xxxxx (회전 동작 회수) , R: xxx (신축 동작 횟수) , Z: xxx (승/하강 동작 횟수) , TM: xx (동작 시간/HR)
		sprintf( FileName , "%04d%02d%02d_AxisCount.log" , st.wYear , st.wMonth , st.wDay );
		if ( strlen(LogDrive) >= 2 ) {//Log드라이브 선택에 따라 다른 경로로 분기되도록 수정(2010.01.25, ybkim)
			sprintf( Path , "DATALOG\\VAC%d\\%s" , Robot_Pos, FileName );
		}
		else if ( strlen(LogDrive) == 1 ) {
...
// Start : 2008.08.17 LOG Folder 미존재 시 자동 Folder 생성 및 예외 처리. ( BY LJH )
	...
	if( !fp ) { 
		...
		if ( Mode == AXIS_LOAD )
		{
			fprintf( fp , "%04d/%02d/%02d %02d:%02d(Ti,A,D,To,B,C,Z)\t" , st.wYear , st.wMonth , st.wDay , st.wHour , st.wMinute );
		}else if ( Mode == AXIS_CNT )
		{
			fprintf( fp , "%04d/%02d/%02d %02d:%02d(N,R,Z,TM(Hour))\t" , st.wYear , st.wMonth , st.wDay , st.wHour , st.wMinute );
		}
...
//	else  TimeOut = RealTimeout;
	// ULVAC Action Command에 대한 Response가 30sec이후에 들어 오는 문제.
	// HOME Action은 실제로 Action Time이 길지만, 간헐적으로 Response가 비정상적으로 늦게 들어와
	// WRITE_DIGITAL API가 False로 Return ( 30sec )되어 버려 Error 발생 관련하여 Normal Timeout을 50sec로 늘리고
	// TM_Rblow Funciont에서 Robot_Sts가 Complete인지 Check하도록 보강.
	if	( !strcmp( Cmnds , "HOME" ) || !strcmp( Cmnds , "HOME2" ) || !strcmp( Cmnds , "HOME3" ) ) TimeOut = 90000;	
	else  TimeOut = 50000 ;
...
BOOL Rs232_IO_Recv( char *Response ) {
	...
	// RS232_Read_String Timeout전에 WRITE_DIGITAL이 False처리 시 Response Null Alarm으로 Post되지 않는 Bug 수정. ( 2013/07/09 , LJH )
	// NullRsp_Chk 1로 초기화. Response가 존재 시 0으로 처리.
	NullRsp_Chk = 1;
...
BOOL Rs232_IO( char *Command , char *Response , BOOL bRead,  int IO_MODE ) {
	...
	if ( IO_MODE == IO_ACTION ) RB_RUN_STS = UV_IDLE ;

	if ( !strcmp ( Command , "GETVCDLOG" )) TimeLog_Axis ( Command , AXIS_LOAD ) ;
	else if ( !strcmp ( Command , "RQ GOTOCOUNTER" )) TimeLog_Axis ( Command , AXIS_CNT ) ;
	else TimeLog( Command );
	...
	if ( Result && bRead )
	{
		if ( IO_MODE == IO_ACTION ) RB_RUN_STS = UV_BUSY ;
		Result = Rs232_IO_Recv( Response ) ;
	}

	if ( !Result ) 
	{
		CommStatus = FALSE;
		if ( IO_MODE == IO_ACTION ) RB_RUN_STS = UV_FAIL ;
	}
	else {
		CommStatus = TRUE;
		GetLocalTime( &st );
		ULVAC_LastPollingTime = SystemTimeToAbsTime( st );
		if( IO_MODE == IO_ACTION ) RB_RUN_STS = UV_COMPLETE ;
	}
	//
	if ( !strcmp ( Command , "GETVCDLOG" )) TimeLog_Axis ( Response , AXIS_LOAD ) ;
	else if ( !strcmp ( Command , "RQ GOTOCOUNTER" )) TimeLog_Axis ( Response , AXIS_CNT ) ;
	else TimeLog( Response );
...
void Analog_Out( int ID1 , int ID2 , int ID3 , double Data , int *Result ) {
	...
	switch( ID1 ) {
		case 22 :	//2008.06.10 AWS Correctable Limit High
			...
			//		printf ( "\n SendBuf[%s] \n" , SendBuf ) ;
					*Result = Rs232_IO( SendBuf , RecvBuf , TRUE , IO_REQUEST ) ;
					if ( *Result ) {
			//			printf ( "\n RecvBuf[%s] \n" , RecvBuf ) ;						
					}
	...
	switch( ID1 ) {
		case 1	 :	// Servo On/Off
					...
					*Result = Rs232_IO( SendBuf , RecvBuf , TRUE , IO_REQUEST ) ;
					break ;
		case 2	 :	// Stop
					*Result = Rs232_IO( "HALT" , RecvBuf , TRUE , IO_REQUEST ) ;
					break ;
		...
		case 234  :	//2008.06.10 AWS Read
					if		( ID2 == 1 ) *Result =  Rs232_IO( "RQ WFOST A" , RecvBuf , TRUE , IO_REQUEST ) ;
					else if	( ID2 == 2 ) *Result =  Rs232_IO( "RQ WFOST B" , RecvBuf , TRUE , IO_REQUEST ) ;
					else if	( ID2 == 3 ) *Result =  Rs232_IO( "RQ WFOST C" , RecvBuf , TRUE , IO_REQUEST ) ;
					else if	( ID2 == 4 ) *Result =  Rs232_IO( "RQ WFOST D" , RecvBuf , TRUE , IO_REQUEST ) ;
		...
		case 235  :	//2008.06.10 AWS Result
					if		( ID2 == 1 ) *Result =  Rs232_IO( "RQ ASTS A 2" , RecvBuf , TRUE , IO_REQUEST ) ;
					else if	( ID2 == 2 ) *Result =  Rs232_IO( "RQ ASTS B 2" , RecvBuf , TRUE , IO_REQUEST ) ;
					else if	( ID2 == 3 ) *Result =  Rs232_IO( "RQ ASTS C 2" , RecvBuf , TRUE , IO_REQUEST ) ;	
					else if	( ID2 == 4 ) *Result =  Rs232_IO( "RQ ASTS D 2" , RecvBuf , TRUE , IO_REQUEST ) ;
...
void String_In( int ID1 , char *data , int *Result ) {
	...
	*Result = FALSE;
	...
	switch ( ID1 ) {
		...
		case 2   :
					*Result = Rs232_IO( "GETVCDLOG" , RecvBuf , TRUE , IO_STATUS ) ;
		...
		case 3   :
					*Result = Rs232_IO( "RQ GOTOCOUNTER" , RecvBuf , TRUE , IO_STATUS ) ;
					sprintf( data , "%s" , RecvBuf );
					break;
		default  :	
					*Result = FALSE ;
