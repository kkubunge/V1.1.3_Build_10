...
enum   {eNOTUSE_0,	eUSE_1}; 

//-----------------------------------------------------------------------------------
#define IO_DATA_FILE_NAME	".\\Parameter\\EQ_SETTING\\YASKAWA_CFG.dat"
int g_nAlignerUse		  = 0;
//-----------------------------------------------------------------------------------
...
BOOL LoadIOData()
{
	...
	do {
		...
		do {
			tp.SetText(szRead);
			tp.GetWord(szItem);
			if(strcmp(szItem, "AlignerUSE") == 0) { tp.GetWord(szAlinger); g_nAlignerUse = atoi(szAlinger);}
			if(NULL == fgets(szRead, 255, fp)) break;
		} while(feof(fp) == 0);
		fclose(fp);
		bRet = TRUE;
...
BOOL Device_Attach( char *Name , char *File , int ID1 , int ID2 , int ID3 , int ID4 , int ID5 , int ID6 , int ID7 , int ID8 , int ID9 , int ID10 ) {
	...
	if (RS232_Connect_Port( ID1 , ID2 , ID3 , ID4 , ID5 , ID6 , ID7 , ID8 ) ) {
		...
		//... 2018.01.23 SW Loading 시 실행되는 모든 동작을 차단한다.
		...
		if (FALSE == Send_Motion_Command(SBuffer , szCmd , g_nRobotUnit , TRUE))
		{
			...
			//return FALSE;   //... 2017.12.23 제조 및 Setting 변위 위해 Project Loading시 ATM 동작 상태 Check 하는 부분 삭제 한다.
		}

		//... 2017.12.23 Add Aligner Use/NotUse Option 
		if(g_nAlignerUse != eNOTUSE_0 && g_nAlignerUse != eUSE_1) g_nAlignerUse = eNOTUSE_0;

		if(g_nAlignerUse == eUSE_1)
		{
