...
//-----------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------
BOOL Rs232_IO_Send(char *Cmnds, BOOL bMotion) {

	//////////////////////////////////////////////////////////////////////////
	//... DEBUG
	printf("[YASKAWA] Send Cmnds[%s]\n", Cmnds);
	
	if(bMotion)		_sleep(1800);
...
BOOL Send_Motion_Command(char* SendData , char* Cmnds , int nUnitNum , BOOL bACK) 
{
	...
	do {
		if (FALSE ==  Rs232_IO_Send(SendData, TRUE)) break;			
		bRet = TRUE;
	} while(0);

	_LOG("Finished 'Send_Motion_Command': '%s'. (%d)", Cmnds, __LINE__);

	g_nRobotStatus = ROBOT_IDLE;
...
BOOL Send_RQ_Command(char* SendData , char* Cmnds) 
{
	...
	do {
		bRet = Rs232_IO_Send(SendData, FALSE); 
		if (FALSE == bRet) break;
	} while(0);
