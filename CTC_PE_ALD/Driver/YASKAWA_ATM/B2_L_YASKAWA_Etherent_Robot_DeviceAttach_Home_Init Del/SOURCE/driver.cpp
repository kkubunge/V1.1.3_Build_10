	...
	do {
		...
		if (NULL == szTemp) {
			...
			if      ('?' == szResponse[0]) {
				...
				if (0 == strcmp(szCmnds , "RSTS"))		bRet = ParseRSTSStatus(szResponse);				
				break;
			}	
			else {			
				if (nElapseTime > g_nTimeOut) {
...
BOOL Send_Motion_Command(char* SendData , char* Cmnds , int nUnitNum , BOOL bACK) 
{
	...
	do {
		...
		{
			printf("[ERR]YASKAWA Robot Ethernet Connect Failed\n");
			CommStatus = FALSE;
			break;
		}
...
BOOL Send_RQ_Command(char* SendData , char* Cmnds) 
{
	...
	do {
		...
		{
			printf("[ERR]YASKAWA Robot Ethernet Connect Fail\n");
			CommStatus = FALSE;
			break;
		}
	} while(0);
