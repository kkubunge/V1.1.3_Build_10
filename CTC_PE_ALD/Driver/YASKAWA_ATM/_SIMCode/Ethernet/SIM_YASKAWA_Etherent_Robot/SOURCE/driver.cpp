...
int		g_nPortNum = 10110;		//... Robot Fixed
...
BOOL Net_IO_Send(char *Cmnds ) {
	...
	//////////////////////////////////////////////////////////////////////////
	//... DEBUG
	printf("[YASKAWA] Robot Send Cmnds[%s]\n", Cmnds);
...
char* GetArmName(int nArm , int nAct)
{
	...
	switch (nArm) {
	case 0: //Arm A
		if (nAct == PICK)		strcpy(g_szArmName , "GA");
		else					strcpy(g_szArmName , "PA");
		g_nCurrentArm = HAND_A;
		break;
	case 1: //Arm B
		if (nAct == PICK)		strcpy(g_szArmName , "GB");
		else					strcpy(g_szArmName , "PB");
		g_nCurrentArm = HAND_B;
		break;
	case 2: //Dual Arm
		if (nAct == PICK)		strcpy(g_szArmName , "GW");
		else					strcpy(g_szArmName , "PW");
...
BOOL Device_Attach( char *Name , char *File , int ID1 , int ID2 , int ID3 , int ID4 , int ID5 , int ID6 , int ID7 , int ID8 , int ID9 , int ID10 ) {
	...
	//////////////////////////////////////////////////////////////////////////
	//... ATM Robot Error Clear at the beginning of driver initialize
	memset(SBuffer, 0, sizeof(SBuffer));
	strcpy(szCmd, "INIT");
	sprintf(SBuffer, "$%d%s10", g_nRobotUnit, szCmd);
	if(FALSE == Send_Motion_Command(SBuffer, szCmd, g_nRobotUnit, TRUE))
	{
		printf("[ERR]YASKAWA Robot Initialize Failed...\n");
		return FALSE;
	}
	else
	{
		printf("[INFO]YASKAWA Robot Initialize Completed...\n");
	}
	...
	if(FALSE == Send_Motion_Command(SBuffer, szCmd, g_nRobotUnit, TRUE))
	{
		printf("[ERR]YASKAWA Robot Home All Failed...\n");
		return FALSE;
	}
	else
	{
		printf("[INFO]YASKAWA Robot Home Completed...\n");
	}
