...
BOOL ReadConfigration()
{
	...
	_LOG("#####> LotPreRcpN:%d, AutoCleaningType:%d " , g_nLotPreNum, g_nCleanType);
	if(bRet == FALSE)
	{
		...
		if(0 == side || 1 == side || 2 == side) // CM1   gPreviousBefore_Route,gPrevious_Route,gCurrent_Route
		{
			...
			if((nRunSts > 3 && nRunSts < 16) || (nRunSts2 > 3 && nRunSts2 < 16) )
			{
				if((gLCStart - 1) >= 1)
				{
					...
					{
						case 0:
							strcpy(gRunning_Route,gCM1_Route);
							break;
						case 1:
							strcpy(gRunning_Route,gCM2_Route);
							break;
						case 2:
							strcpy(gRunning_Route,gCM3_Route);
							break;
					}
			...
			{
				case 0:
					strcpy(gLocal_Route,gCM1_Route);
					break;
				case 1:
					strcpy(gLocal_Route,gCM2_Route);
					break;
				case 2:
					strcpy(gLocal_Route,gCM3_Route);
					break;
			}
			...
			else // One Prevoius Lot Running
			{
				do{
						...
						{  	
							...
							/*
							//// Route Sharing Check 2 2006.12.11 ///////////////////////////////////////////
							if(bIdentical != TRUE && (nCurrentRunningLotPreType == nStartNowLotPreType))
							{
								if( strlen(gCurrent_Route) <= strlen(gRunning_Route) )
								{
									startmode = 1;		// 0 - Concurrent ,1 - Continue , 2 - Sequence
									_LOG("GetLotStartOption: 1 Not-Concurrent LotPreType Same In 2 Lot => Continue");
									break;
								}
							}
							//// Route Sharing Check 2 2006.12.11 ///////////////////////////////////////////
						...
						{	    // Partial Sharing => Sequential
								...
								else
								{
									...
									break;
//									startmode = 2;		// 0 - Concurrent ,1 - Continue , 2 - Sequence
//									_LOG( "GetLotStartOption:  2Lot_Run_Partial_Same Current_LotPre_Use =>  Sequence");
								}
//								break;
	...
	{
		startmode = 1;		// 0 - Concurrent ,1 - Continue , 2 - Sequence
		_LOG( "GetLotStartOption: No Continus Lot =>  Continue");
	}
	...
	for(i = 1; i <=3 ; i++)
	{
		...
		{
			case 0:
				strcpy(strRoute,gCM1_Route);
				break;
			case 1:
				strcpy(strRoute,gCM2_Route);
				break;
			case 2:
				strcpy(strRoute,gCM3_Route);
				break;
		}
		if(i == 1)
			strcpy(strRoute1,strRoute);
		else if(i == 2)
			strcpy(strRoute2,strRoute);
		else if( i == 3)
			strcpy(strRoute3,strRoute);
	}
	if(startmode == 0)
		sprintf(strStartMode,"ConCurrent.");
	else if(startmode == 1)
		sprintf(strStartMode,"Continue.  ");
	else if(startmode == 2)
		sprintf(strStartMode,"Sequential.");
	...
	if(nPort == 0 || nPort == 1 || nPort == 2)
	{
		...
		if((nRunSts > 3 && nRunSts < 16) || (nRunSts2 > 3 && nRunSts2 < 16))// Previous Lot Running
		{
			do
			{
				...
				{
					if((gLCStart - 1) >= 1)
						nRunningLotPreType = gCMLotPre[gLCStart-1];
					else
						nRunningLotPreType = gCMLotPre[gLCStart];
...
/*				// 아래 부분은 기존 Code 임.
				if(nPort == 0)
				{
					// Check if CM2 Running
					if(nRunSts > 3 && nRunSts < 16) // 7 == Lot Starting, 15 == Run Complete 
					{
						nRunningLotPreType = gCM2LotPreType;
						_LOG( "[FUNCTION] nRunningLotPreType of CM2 is %d" , nRunningLotPreType);
					}
					// Check if CM3 Running
					if(nRunSts2 > 3 && nRunSts2 < 16) // 7 == Lot Starting, 15 == Run Complete 
					{
						nRunningLotPreType = gCM3LotPreType;
						_LOG( "[FUNCTION] nRunningLotPreType of CM3 is %d" , nRunningLotPreType);
					}
					
				}
				else if(nPort == 1) // CM2
				{
					// Check if CM1 Running
					if(nRunSts > 3 && nRunSts < 16) // 7 == Lot Starting, 15 == Run Complete 
					{
						nRunningLotPreType = gCM1LotPreType;
						_LOG( "[FUNCTION] nRunningLotPreType of CM1 is %d" , nRunningLotPreType);
					}
					// Check if CM3 Running
					if(nRunSts2 > 3 && nRunSts2 < 16) // 7 == Lot Starting, 15 == Run Complete 
					{
