...
bool CTextFile::Open(const char *szFileName, int nMode)
{
	...
	{
	case TEXT_FILE_READ:
		m_pFP = fopen(szFileName, "rt");
		break;
	case TEXT_FILE_WRITE:
		m_pFP = fopen(szFileName, "r+t");
		break;
	case TEXT_FILE_CREATE:
		m_pFP = fopen(szFileName, "w+t");
		break;
	case TEXT_FILE_APPEND:
		m_pFP = fopen(szFileName, "a+t");
		break;
	default:
		bRet = false;
		break;
...
{
	if(m_pFP != NULL) fclose((FILE *)m_pFP);
	m_pFP = NULL;
	return true;
}
...
bool CTextFile::ReadWord(char *pBuf, char cToken)
{
	...
	*pBuf = NULL;
	...
	do {
		if( feof((FILE *)m_pFP) ) break;
		szBuf[0] = fgetc((FILE *)m_pFP);
	} while(' ' == szBuf[0] || '\n' == szBuf[0]
...
bool CTextFile::WriteString(char *pBuf)
{
	bool bRet = true;
...
bool CTextFile::WriteLine(char *pBuf)
{
	bool bRet = false;
	fputs(pBuf, (FILE *)m_pFP);
	fputc('\n', (FILE *)m_pFP);
...
{
	if( feof((FILE *)m_pFP) != 0 ) return true;
	return false;
}
