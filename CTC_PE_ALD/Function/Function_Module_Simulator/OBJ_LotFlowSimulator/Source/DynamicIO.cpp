...
// 2011.08.18
char* CDynamicIO::dREAD_IO_FROM_NAME(char *szIOName, int nIOType, char *szValue, BOOL *pResult)
{
	int nIOPoint;
	int nValue;
	double dbValue;
	...
	strcpy(szValue, "");
	nIOPoint = _FIND_FROM_STRING(nIOType, szIOName);
	if(-1 == nIOPoint) { *pResult = FALSE; return szValue; }

	switch(nIOType)
	{
	case _K_D_IO: nValue = _dREAD_DIGITAL(nIOPoint, pResult); sprintf(szValue, "%d", nValue); break;
	case _K_A_IO: dbValue = _dREAD_ANALOG(nIOPoint, pResult); sprintf(szValue, "%.3f", dbValue); break;
	case _K_S_IO: _dREAD_STRING(nIOPoint, szValue, pResult); break;
