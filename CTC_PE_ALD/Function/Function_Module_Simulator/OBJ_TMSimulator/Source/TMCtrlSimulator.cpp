...
//	{ "TM.PM4.SlotVv1XO"			, _K_D_IO,	0	} ,
//	{ "TM.PM4.SlotVv2XO"			, _K_D_IO,	0	} ,
//	{ "TM.PM5.SlotVv1XO"			, _K_D_IO,	0	} ,
//	{ "TM.PM5.SlotVv2XO"			, _K_D_IO,	0	} ,
...
//	{ "TM.PM4.SlotVv1XI"			, _K_D_IO,	0	} ,
//	{ "TM.PM4.SlotVv2XI"			, _K_D_IO,	0	} ,
//	{ "TM.PM5.SlotVv1XI"			, _K_D_IO,	0	} ,
//	{ "TM.PM5.SlotVv2XI"			, _K_D_IO,	0	} ,
...
/*	{"ATM_ROBOT READY",					1.0},
	...
	{"ATM_ROBOT ROTATE",				1.0},
	{"ATM_ROBOT PICK CM",				4.0},
	{"ATM_ROBOT PLACE CM",				4.0},
	{"ATM_ROBOT PICK BM",				4.0},
	{"ATM_ROBOT PLACE BM",				4.0},
	{"ATM_ROBOT PICK AL",				4.0},
	{"ATM_ROBOT PLACE AL",				4.0},
	{"ALIGNER",							0.8},
	// 2006.01.05
	{"ATM_ROBOT ROTPIC CM",				4.0},
	{"ATM_ROBOT ROTPLC CM",				4.0},
	{"ATM_ROBOT ROTPIC BM",				4.0},
	{"ATM_ROBOT ROTPLC BM",				4.0},
	{"ATM_ROBOT ROTPIC AL",				4.0},
	{"ATM_ROBOT ROTPLC AL",				4.0},
*/
	{"ATM_ROBOT READY",					1.0},
	{"ATM_ROBOT MAPPING",				5.0},
	{"ALIGNER",							3.8},
...
BOOL CTMCtrlSimulator::Main(char *szParam)
{
	...
	//else if(tp.BeEqual("MAINBM"))		bRet = ControlMainBM(szParam);
...
/*	else if(tp.BeEqual(szItem, "ATM_Robot_Rotate_Time"))		pCmd = "ATM_ROBOT ROTATE";
	else if(tp.BeEqual(szItem, "ATM_Robot_Pick_CM_Time"))		pCmd = "ATM_ROBOT PICK CM";
	else if(tp.BeEqual(szItem, "ATM_Robot_Place_CM_Time"))		pCmd = "ATM_ROBOT PLACE CM";
	else if(tp.BeEqual(szItem, "ATM_Robot_Pick_BM_Time"))		pCmd = "ATM_ROBOT PICK BM";
	else if(tp.BeEqual(szItem, "ATM_Robot_Place_BM_Time"))		pCmd = "ATM_ROBOT PLACE BM";
	else if(tp.BeEqual(szItem, "ATM_Robot_Pick_Aligner_Time"))	pCmd = "ATM_ROBOT PICK AL";
	else if(tp.BeEqual(szItem, "ATM_Robot_Place_Aligner_Time")) pCmd = "ATM_ROBOT PLACE AL";
	else if(tp.BeEqual(szItem, "ALGN_Align_Time"))				pCmd = "ALIGNER";
	// 2006.01.05
	else if(tp.BeEqual(szItem, "ATM_Robot_RotPick_CM_Time"))		pCmd = "ATM_ROBOT ROTPIC CM";
	else if(tp.BeEqual(szItem, "ATM_Robot_RotPlace_CM_Time"))		pCmd = "ATM_ROBOT ROTPLC CM";
	else if(tp.BeEqual(szItem, "ATM_Robot_RotPick_BM_Time"))		pCmd = "ATM_ROBOT ROTPIC BM";
	else if(tp.BeEqual(szItem, "ATM_Robot_RotPlace_BM_Time"))		pCmd = "ATM_ROBOT ROTPLC BM";
	else if(tp.BeEqual(szItem, "ATM_Robot_RotPick_Aligner_Time"))	pCmd = "ATM_ROBOT ROTPIC AL";
	else if(tp.BeEqual(szItem, "ATM_Robot_RotPlace_Aligner_Time"))	pCmd = "ATM_ROBOT ROTPLC AL";
*/
	else if(tp.BeEqual(szItem, "ALGN_Align_Time")) 			  pCmd = "ALIGNER";
...
/*BOOL CTMCtrlSimulator::ControlMainBM(char *szCmd)
{
	BOOL bRet = FALSE;
	BOOL bIOStatus;
	double dbWaitTime = 0;
	...
	do {
		...
		nSlowRoughValve = SlowRoughVvDO_BM;
		nFastRoughValve = FastRoughVvDO_BM;
		nSlowVentValve  = SlowVentVvDO_BM;
		nFastVentValve  = FastVentVvDO_BM;
		nChmPrsAI       = BM_CGChmPrsAI;
		nATMVACSensorXI = BM_ATMVACSensorXI;
		nVacuumStatusDM = BM_VacuumStatusDM;
		strcpy(szModule, "MAINBM");
		...
		if(tp.BeEqual("PUMP"))
		{
			...
			sprintf(szTemp, "ISO_GATE CLOSE %s", szModule);
			if(! ControlIsoGate(szTemp)) break;
...
BOOL CTMCtrlSimulator::ControlIsoGate(char *szCmd)
{
	...
	do {
		...
		} /*else if(tp.BeEqual("PM4")) { 
			nSlotVvXO  = PM4_SlotValve1XO; 
			nSlotVvXI  = PM4_SlotValve1XI;
			nSlotVv2XO = PM4_SlotValve2XO; 
			nSlotVv2XI = PM4_SlotValve2XI;
		} else if(tp.BeEqual("PM5")) { 
			nSlotVvXO  = PM5_SlotValve1XO; 
			nSlotVvXI  = PM5_SlotValve1XI; 
			nSlotVv2XO = PM5_SlotValve2XO; 
			nSlotVv2XI = PM5_SlotValve2XI; 
		}*/
		else break;
