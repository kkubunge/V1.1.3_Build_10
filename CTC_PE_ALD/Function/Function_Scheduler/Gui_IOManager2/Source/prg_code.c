...
BOOL Check_IOType(int Find, char *Buffer, int nIOType)
{
	...
	if(nLenth > 3)
	{
		...
		sprintf(szExcept3, "MTR7.Set_Load_");

		// 2015.01.10 Exceptional IO Check by hyuk
		switch(nIOType)
		{
			case 0:	//Digital
				if(NULL != strstr(Buffer, szExcept3))
				{
					return TRUE;
				}	
				break;
...
//			if ( D_IO_TYPE( k ) == OUTPUT_IO_TYPE ) { // 2003.08.26
			...
			if ( AllMode || ( DIGITAL_DATA_BUFFER_READ[ j ] != D_IO_IN_VALUE(k) ) ) {
				KGUI_DRAW_StrBox22( hDC , DISPLAY_START_X_FOR_DIGITAL3 , DISPLAY_START_Y_FOR_DIGITAL + ( j * DISPLAY_SIZE_Y_FOR_DIGITAL ) , D_IO_IN_VALUE_STR(k) , DISPLAY_SIZE_X_FOR_DIGITAL3 , 1 , COLOR_DarkGray , COLOR_White , COLOR_Black , 13 );		//Input
