...
int  DATALOG_STRING_IO3; 
int  DATALOG_STRING_IO4; 
...
void Elapsed_Time_with_Datalog( int mode , char *buffer ) {
	SYSTEMTIME SysTime; // 2010.05.26
	time_t finish;
	long elapsed_time ; //, hour , min , sec;
	//
	switch ( mode ) { // 2010.05.26
	case 4 :
	case 5 :
...
{
	char Find_Folder_Address[256];
	int nResult;
	SYSTEMTIME st;
	GetLocalTime( &st );
	sprintf( Find_Folder_Address , "%s/%04d%02d%02d" ,DATALOG_DIRECTORY2,  st.wYear , st.wMonth , st.wDay );
	nResult = access( Find_Folder_Address, 0 );
	if(nResult == -1)
	{
		printf("Directory Absent\n");
		CreateDirectory ( DATALOG_DIRECTORY , NULL );
	}
	else if ( nResult == 0)
	{
		printf("Directory Present\n");
	}
	sprintf( DATALOG_DIRECTORY , "%s/%04d%02d%02d", DATALOG_DIRECTORY2, st.wYear , st.wMonth , st.wDay );
...
//	fprintf( fpt , "$_DATALOG_TIME  %4d\n" , DATALOG_TIME ); // 2013.07.05
	fprintf( fpt , "$_DATALOG_TIME  %.2f\n" , DATALOG_TIME ); // 2013.07.05
...
BOOL READ_AIO_INFORMATION( char *Name , char *Show , int Point , char *Help , BOOL Ana , int Tgl , BOOL remotemode ) {
	...
	int Out_Dev;
	char EnumList[1024];
	memset(EnumList			,0x00, sizeof(EnumList));
	...
	for ( Line = 1 ; FileEnd ; Line++ ) {
		...
		if ( ReadCnt > 0 ) {
			...
			//	if ( !Get_Data_From_String_for_INT( Buffer , 1 , chr_return1 , 0 , 999 , &iRes ) ) {
				if ( !Get_Data_From_String_for_DOUBLE( Buffer , 1 , chr_return1 , 0 , 999 , &dRes ) ) { // 2013.07.05
...
BOOL Read_Information_File( char *FileName , BOOL GuiDraw ) {
	...
	memset(chr_return3		,0x00, sizeof(chr_return3));
	...
	DATALOG_STRING_IO3			= -1;
	DATALOG_STRING_IO4			= -1;
	//---> END    2008/08/27 HeaderItem 추가 by KSM
	...
	for ( Line = 1 ; FileEnd ; Line++ ) {
		...
		if ( ReadCnt > 0 ) {
			...
			*/
			...
			else if ( STRCMP_L( "$DATALOG_STRING_IO3" , chr_return1 ) ) { 
				if ( !REMOTE_MONITOR_MODE ) {
					if ( !Get_Data_From_String( Buffer , chr_return2 , 1 ) ) {
						_IO_CIM_PRINTF( "Unknown Error 1 $DATALOG_STRING_IO3 - [%s]-[%d]\n" , FileName , Line );
						_lclose( hFile );
						return FALSE;
					}
					DATALOG_STRING_IO3 = _FIND_FROM_STRING( _KG_S_IO , chr_return2 );
					if ( DATALOG_STRING_IO3 == -1 ) { 
						_IO_CIM_PRINTF( "Unknown Object name in Data IO Table - $DATALOG_STRING_IO3 %s\n" , chr_return2 );
						_lclose( hFile );
						return FALSE;
					}
				}
			}
			else if ( STRCMP_L( "$DATALOG_STRING_IO4" , chr_return1 ) ) { 
				if ( !REMOTE_MONITOR_MODE ) {
					if ( !Get_Data_From_String( Buffer , chr_return2 , 1 ) ) {
						_IO_CIM_PRINTF( "Unknown Error 1 $DATALOG_STRING_IO4 - [%s]-[%d]\n" , FileName , Line );
						_lclose( hFile );
						return FALSE;
					}
					DATALOG_STRING_IO4 = _FIND_FROM_STRING( _KG_S_IO , chr_return2 );
					if ( DATALOG_STRING_IO4 == -1 ) { 
						_IO_CIM_PRINTF( "Unknown Object name in Data IO Table - $DATALOG_STRING_IO4 %s\n" , chr_return2 );
						_lclose( hFile );
						return FALSE;
					}
				}
			...
			//---> End 2008/08/27 HeaderItem 추가 by KSM
				...
				if ( !REMOTE_MONITOR_MODE ) { // 2005.03.15
//					if ( !Get_Data_From_String_for_INT( Buffer , 1 , chr_return1 , 0 , 999 , &DATALOG_TIME ) ) { // 2013.07.05
					if ( !Get_Data_From_String_for_DOUBLE( Buffer , 1 , chr_return1 , 0 , 999 , &DATALOG_TIME ) ) { // 2013.07.05
			...
			else if ( STRCMP_L( "$DATALOG_DIRECTORY" , chr_return1 ) ) 
			{
				...
				sprintf(DATALOG_DIRECTORY2, DATALOG_DIRECTORY);
