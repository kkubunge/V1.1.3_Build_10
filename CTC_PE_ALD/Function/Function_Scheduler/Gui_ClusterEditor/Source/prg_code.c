...
//------------------------------------------------------------------------------------------
#define BUTTON_X_INS_START			708
#define BUTTON_Y_INS_START			24

#define BUTTON_X_REMOVE_START		798
#define BUTTON_Y_REMOVE_START		24

#define BUTTON_X_SELECT_START		554
#define BUTTON_Y_SELECT_START		24
//------------------------------------------------------------------------------------------
//
//
...
void FILE_HANDLING_BUTTON_MAKE( HWND hdlg , int Init ) {
	int			F_I , i , ae , ad , ap , ce , cd , cp , xs , ys , xp , yp;
	RECT rect;

	//------------------------------------------------------------------------------------------------------------------
	if ( Init == 1 ) {
		if ( Get_Prm_TITLE_MODE() ) {
			GetWindowRect( hdlg , &rect ); // 2005.05.03
			xp = rect.left; // 2005.05.03
			yp = rect.top; // 2005.05.03
			
			GetWindowRect( GetDlgItem( hdlg , IDC_CURRENT_CLUSTERFILE ) , &rect );
			i = rect.top;
			KWIN_Item_Resize( hdlg , IDC_CURRENT_CLUSTERFILE , rect.left - xp , i + 40 - yp , rect.right - rect.left + 1 , rect.bottom - rect.top + 1 );
			//
			GetWindowRect( GetDlgItem( hdlg , IDC_BUTTON1 ) , &rect );
			ae = rect.top - i;
			if ( ae < 0 ) {
				if ( !Get_Prm_GROUPBUTTONHIDE_MODE() ) { // 2002.11.14
					KWIN_Item_Resize( hdlg , IDC_BUTTON1 , rect.left - xp , rect.top + 40 - yp , rect.right - rect.left + 1 , rect.bottom - rect.top + 1 );
				}
				//
				GetWindowRect( GetDlgItem( hdlg , IDC_LIST_RECIPEDIR ) , &rect );
				KWIN_Item_Resize( hdlg , IDC_LIST_RECIPEDIR , rect.left - xp , rect.top + 40 - yp , rect.right - rect.left + 1 , rect.bottom - rect.top + 1 );
			}
			else {
				if ( !Get_Prm_GROUPBUTTONHIDE_MODE() ) { // 2002.11.14
					KWIN_Item_Resize( hdlg , IDC_BUTTON1 , rect.left - xp , i + 40 - yp , rect.right - rect.left + 1 , rect.bottom - rect.top + 1 + ae );
				}
				//
				GetWindowRect( GetDlgItem( hdlg , IDC_LIST_RECIPEDIR ) , &rect );
				KWIN_Item_Resize( hdlg , IDC_LIST_RECIPEDIR , rect.left - xp , i + 40 - yp , rect.right - rect.left + 1 , rect.bottom - rect.top + 1 + ae );
			}
			//
			GetWindowRect( GetDlgItem( hdlg , IDC_LIST_CLUSTERFILE ) , &rect );
			KWIN_Item_Resize( hdlg , IDC_LIST_CLUSTERFILE , rect.left - xp , rect.top + 40 - yp , rect.right - rect.left + 1 , rect.bottom - rect.top + 1 - 40 );
			//
			GetWindowRect( GetDlgItem( hdlg , IDC_CTRL_HANDLING_SIDE ) , &rect );
			KWIN_Item_Resize( hdlg , IDC_CTRL_HANDLING_SIDE , rect.left - xp , rect.top + 40 - yp , rect.right - rect.left + 1 , rect.bottom - rect.top + 1 );
			//
			GetWindowRect( GetDlgItem( hdlg , IDC_LIST_CLUSTERDATA ) , &rect );
			KWIN_Item_Resize( hdlg , IDC_LIST_CLUSTERDATA , rect.left - xp , rect.top + 40 - yp , rect.right - rect.left + 1 , rect.bottom - rect.top + 1 - 40 );
		}
	}
	else {
	...
	if ( !Get_Prm_MAXVIEW_MODE() ) {
		...
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][0] , F_Y[0][0] , 58 , 58 , hdlg , (HMENU) IDB_NEW_ENABLE , GETHINST( hdlg ) , NULL );	h_button_count++;
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][0] , F_Y[0][0] , 58 , 58 , hdlg , (HMENU) IDB_NEW_DISABLE , GETHINST( hdlg ) , NULL );	h_button_count++;
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][0] , F_Y[0][0] , 58 , 58 , hdlg , (HMENU) IDB_NEW_PRESSED , GETHINST( hdlg ) , NULL );	h_button_count++;
		//
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][1] , F_Y[0][1] , 58 , 58 , hdlg , (HMENU) IDB_SAVE_ENABLE   , GETHINST( hdlg ) , NULL );	h_button_count++;
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][1] , F_Y[0][1] , 58 , 58 , hdlg , (HMENU) IDB_SAVE_DISABLE   , GETHINST( hdlg ) , NULL );	h_button_count++;
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][1] , F_Y[0][1] , 58 , 58 , hdlg , (HMENU) IDB_SAVE_PRESSED   , GETHINST( hdlg ) , NULL );	h_button_count++;
		//
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][2] , F_Y[0][2] , 58 , 58 , hdlg , (HMENU) IDB_SAVEAS_ENABLE  , GETHINST( hdlg ) , NULL );	h_button_count++;
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][2] , F_Y[0][2] , 58 , 58 , hdlg , (HMENU) IDB_SAVEAS_DISABLE  , GETHINST( hdlg ) , NULL );	h_button_count++;
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][2] , F_Y[0][2] , 58 , 58 , hdlg , (HMENU) IDB_SAVEAS_PRESSED  , GETHINST( hdlg ) , NULL );	h_button_count++;
		//
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][3] , F_Y[0][3] , 58 , 58 , hdlg , (HMENU) IDB_CANCEL_ENABLE , GETHINST( hdlg ) , NULL );	h_button_count++;
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][3] , F_Y[0][3] , 58 , 58 , hdlg , (HMENU) IDB_CANCEL_DISABLE , GETHINST( hdlg ) , NULL );	h_button_count++;
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][3] , F_Y[0][3] , 58 , 58 , hdlg , (HMENU) IDB_CANCEL_PRESSED , GETHINST( hdlg ) , NULL );	h_button_count++;
		//
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][4] , F_Y[0][4] , 58 , 58 , hdlg , (HMENU) IDB_FILE_ENABLE , GETHINST( hdlg ) , NULL );	h_button_count++;
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][4] , F_Y[0][4] , 58 , 58 , hdlg , (HMENU) IDB_FILE_DISABLE , GETHINST( hdlg ) , NULL );	h_button_count++;
		h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , F_X[0][4] , F_Y[0][4] , 58 , 58 , hdlg , (HMENU) IDB_FILE_PRESSED , GETHINST( hdlg ) , NULL );	h_button_count++;
	}
	//== Contents Insert / Remove
	if ( Get_Prm_TITLE_MODE() ) i = 40;
	else                        i = 0;
	h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , BUTTON_X_INS_START , BUTTON_Y_INS_START + i , 90 , 40 , hdlg , (HMENU) IDB_INSERT_ENABLE , GETHINST( hdlg ) , NULL );	h_button_count++;
	h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , BUTTON_X_INS_START , BUTTON_Y_INS_START + i , 90 , 40 , hdlg , (HMENU) IDB_INSERT_DISABLE , GETHINST( hdlg ) , NULL );	h_button_count++;
	h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , BUTTON_X_INS_START , BUTTON_Y_INS_START + i , 90 , 40 , hdlg , (HMENU) IDB_INSERT_PRESSED , GETHINST( hdlg ) , NULL );	h_button_count++;
	//
	h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , BUTTON_X_REMOVE_START , BUTTON_Y_REMOVE_START + i , 90 , 40 , hdlg , (HMENU) IDB_REMOVE_ENABLE , GETHINST( hdlg ) , NULL );	h_button_count++;
	h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , BUTTON_X_REMOVE_START , BUTTON_Y_REMOVE_START + i , 90 , 40 , hdlg , (HMENU) IDB_REMOVE_DISABLE , GETHINST( hdlg ) , NULL );	h_button_count++;
	h_button[h_button_count] = CreateWindowEx( 0L , "button"    , "" , WS_CHILD | WS_VISIBLE | BS_OWNERDRAW , BUTTON_X_REMOVE_START , BUTTON_Y_REMOVE_START + i , 90 , 40 , hdlg , (HMENU) IDB_REMOVE_PRESSED , GETHINST( hdlg ) , NULL );	h_button_count++;
...
//------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------
void Set_VERSION_INFO_REFRESH( HWND hdlg , BOOL mode ) { // 2007.09.18
	if ( !Get_Prm_VERSION_MODE() ) return;
	//
	if ( mode ) {
		KWIN_Item_String_Display( hdlg , IDC_CURRENT_LAST_USER , CLUSTER_RECIPE.F_RECIPE_LAST_UPDATE_USER );
		KWIN_Item_String_Display( hdlg , IDC_CURRENT_LAST_LEVEL , CLUSTER_RECIPE.F_RECIPE_LAST_UPDATE_LEVL );
		KWIN_Item_String_Display( hdlg , IDC_CURRENT_LAST_DATE , CLUSTER_RECIPE.F_RECIPE_LAST_UPDATE_DATE );
	}
	else {
		KWIN_Item_String_Display( hdlg , IDC_CURRENT_LAST_USER , "" );
		KWIN_Item_String_Display( hdlg , IDC_CURRENT_LAST_LEVEL , "" );
		KWIN_Item_String_Display( hdlg , IDC_CURRENT_LAST_DATE , "" );
	}
}
//
//------------------------------------------------------------------------------------------
...
void Set_Enable2_PROCESSFILE( HWND hdlg , BOOL data ) {
	if ( Get_Prm_MAXVIEW_MODE() ) return;
	...
	if ( !Get_Prm_MODULE_MODE( 3 ) ) {
		KWIN_Item_Hide( hdlg , IDC_CTRL_RELOAD_PM4 );
		KWIN_Item_Hide( hdlg , IDC_LIST_PM4FILE );
		//
		KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM4 );
		//
	}
	else if ( data ) {
		KWIN_Item_Enable( hdlg , IDC_CTRL_RELOAD_PM4 );
		KWIN_Item_Enable( hdlg , IDC_LIST_PM4FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[3][PM_SUB_CURRENT_POS[3]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM4 );
		else
			KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM4 );
		//
	}
	else {
		KWIN_Item_Disable( hdlg , IDC_CTRL_RELOAD_PM4 );
		KWIN_Item_Disable( hdlg , IDC_LIST_PM4FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[3][PM_SUB_CURRENT_POS[3]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM4 );
		else
	...
	if ( !Get_Prm_MODULE_MODE( 4 ) ) {
		KWIN_Item_Hide( hdlg , IDC_CTRL_RELOAD_PM5 );
		KWIN_Item_Hide( hdlg , IDC_LIST_PM5FILE );
		//
		KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM5 );
		//
	}
	else if ( data ) {
		KWIN_Item_Enable( hdlg , IDC_CTRL_RELOAD_PM5 );
		KWIN_Item_Enable( hdlg , IDC_LIST_PM5FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[4][PM_SUB_CURRENT_POS[4]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM5 );
		else
			KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM5 );
		//
	}
	else {
		KWIN_Item_Disable( hdlg , IDC_CTRL_RELOAD_PM5 );
		KWIN_Item_Disable( hdlg , IDC_LIST_PM5FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[4][PM_SUB_CURRENT_POS[4]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM5 );
		else
	...
	if ( !Get_Prm_MODULE_MODE( 5 ) ) {
		KWIN_Item_Hide( hdlg , IDC_CTRL_RELOAD_PM6 );
		KWIN_Item_Hide( hdlg , IDC_LIST_PM6FILE );
		//
		KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM6 );
		//
	}
	else if ( data ) {
		KWIN_Item_Enable( hdlg , IDC_CTRL_RELOAD_PM6 );
		KWIN_Item_Enable( hdlg , IDC_LIST_PM6FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[5][PM_SUB_CURRENT_POS[5]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM6 );
		else
			KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM6 );
		//
	}
	else {
		KWIN_Item_Disable( hdlg , IDC_CTRL_RELOAD_PM6 );
		KWIN_Item_Disable( hdlg , IDC_LIST_PM6FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[5][PM_SUB_CURRENT_POS[5]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM6 );
		else
	...
	if ( !Get_Prm_MODULE_MODE( 6 ) ) {
		KWIN_Item_Hide( hdlg , IDC_CTRL_RELOAD_PM7 );
		KWIN_Item_Hide( hdlg , IDC_LIST_PM7FILE );
		//
		KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM7 );
		//
	}
	else if ( data ) {
		KWIN_Item_Enable( hdlg , IDC_CTRL_RELOAD_PM7 );
		KWIN_Item_Enable( hdlg , IDC_LIST_PM7FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[6][PM_SUB_CURRENT_POS[6]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM7 );
		else
			KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM7 );
		//
	}
	else {
		KWIN_Item_Disable( hdlg , IDC_CTRL_RELOAD_PM7 );
		KWIN_Item_Disable( hdlg , IDC_LIST_PM7FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[6][PM_SUB_CURRENT_POS[6]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM7 );
		else
	...
	if ( !Get_Prm_MODULE_MODE( 7 ) ) {
		KWIN_Item_Hide( hdlg , IDC_CTRL_RELOAD_PM8 );
		KWIN_Item_Hide( hdlg , IDC_LIST_PM8FILE );
		//
		KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM8 );
		//
	}
	else if ( data ) {
		KWIN_Item_Enable( hdlg , IDC_CTRL_RELOAD_PM8 );
		KWIN_Item_Enable( hdlg , IDC_LIST_PM8FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[7][PM_SUB_CURRENT_POS[7]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM8 );
		else
			KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM8 );
		//
	}
	else {
		KWIN_Item_Disable( hdlg , IDC_CTRL_RELOAD_PM8 );
		KWIN_Item_Disable( hdlg , IDC_LIST_PM8FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[7][PM_SUB_CURRENT_POS[7]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM8 );
		else
	...
	if ( !Get_Prm_MODULE_MODE( 8 ) ) {
		KWIN_Item_Hide( hdlg , IDC_CTRL_RELOAD_PM9 );
		KWIN_Item_Hide( hdlg , IDC_LIST_PM9FILE );
		//
		KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM9 );
		//
	}
	else if ( data ) {
		KWIN_Item_Enable( hdlg , IDC_CTRL_RELOAD_PM9 );
		KWIN_Item_Enable( hdlg , IDC_LIST_PM9FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[8][PM_SUB_CURRENT_POS[8]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM9 );
		else
			KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM9 );
		//
	}
	else {
		KWIN_Item_Disable( hdlg , IDC_CTRL_RELOAD_PM9 );
		KWIN_Item_Disable( hdlg , IDC_LIST_PM9FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[8][PM_SUB_CURRENT_POS[8]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM9 );
		else
	...
	if ( !Get_Prm_MODULE_MODE( 9 ) ) {
		KWIN_Item_Hide( hdlg , IDC_CTRL_RELOAD_PM10 );
		KWIN_Item_Hide( hdlg , IDC_LIST_PM10FILE );
		//
		KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM10 );
		//
	}
	else if ( data ) {
		KWIN_Item_Enable( hdlg , IDC_CTRL_RELOAD_PM10 );
		KWIN_Item_Enable( hdlg , IDC_LIST_PM10FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[9][PM_SUB_CURRENT_POS[9]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM10 );
		else
			KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM10 );
		//
	}
	else {
		KWIN_Item_Disable( hdlg , IDC_CTRL_RELOAD_PM10 );
		KWIN_Item_Disable( hdlg , IDC_LIST_PM10FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[9][PM_SUB_CURRENT_POS[9]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM10 );
		else
	...
	if ( !Get_Prm_MODULE_MODE( 10 ) ) {
		KWIN_Item_Hide( hdlg , IDC_CTRL_RELOAD_PM11 );
		KWIN_Item_Hide( hdlg , IDC_LIST_PM11FILE );
		//
		KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM11 );
		//
	}
	else if ( data ) {
		KWIN_Item_Enable( hdlg , IDC_CTRL_RELOAD_PM11 );
		KWIN_Item_Enable( hdlg , IDC_LIST_PM11FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[10][PM_SUB_CURRENT_POS[10]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM11 );
		else
			KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM11 );
		//
	}
	else {
		KWIN_Item_Disable( hdlg , IDC_CTRL_RELOAD_PM11 );
		KWIN_Item_Disable( hdlg , IDC_LIST_PM11FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[10][PM_SUB_CURRENT_POS[10]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM11 );
		else
	...
	if ( !Get_Prm_MODULE_MODE( 11 ) ) {
		KWIN_Item_Hide( hdlg , IDC_CTRL_RELOAD_PM12 );
		KWIN_Item_Hide( hdlg , IDC_LIST_PM12FILE );
		//
		KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM12 );
		//
	}
	else if ( data ) {
		KWIN_Item_Enable( hdlg , IDC_CTRL_RELOAD_PM12 );
		KWIN_Item_Enable( hdlg , IDC_LIST_PM12FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[11][PM_SUB_CURRENT_POS[11]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM12 );
		else
			KWIN_Item_Hide( hdlg , IDC_CTRL_CHANGE_PM12 );
		//
	}
	else {
		KWIN_Item_Disable( hdlg , IDC_CTRL_RELOAD_PM12 );
		KWIN_Item_Disable( hdlg , IDC_LIST_PM12FILE );
		//
		if ( strlen( PM_SUB_DIRECTORY[11][PM_SUB_CURRENT_POS[11]] ) > 0 )
			KWIN_Item_Enable( hdlg , IDC_CTRL_CHANGE_PM12 );
		else
...
void EXTRA_STATION_MAPPING( HWND hdlg ) {
	...
	{
	case 10 : KWIN_Item_String_Display( hdlg , IDC_CTRL_HANDLING_SIDE , "Wafer Align");		break;
	case 50 : KWIN_Item_String_Display( hdlg , IDC_CTRL_HANDLING_SIDE , "Align Skip");		break;
	default:
		CLUSTER_RECIPE.EXTRA_STATION = 10;
		KWIN_Item_String_Display( hdlg , IDC_CTRL_HANDLING_SIDE , "Wafer Align" );
		break;
	}
}
...
void Process_Recipe_Directory_Refresh( HWND hdlg , int mode ) {
	if ( mode >= 0 ) {
		...
		if ( Get_Prm_MAXVIEW_MODE() ) {
			if ( PM_LOW_CONTROL[MAX_VIEW_CHAMBER] ) {
				FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PMFILE ) , Get_Prm_PROCESS_RECIPE_PATH( MAX_VIEW_CHAMBER ) , PM_SUB_DIRECTORY[MAX_VIEW_CHAMBER][ PM_SUB_CURRENT_POS[MAX_VIEW_CHAMBER] ] , PM_LOW_DIRECTORY[MAX_VIEW_CHAMBER][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[MAX_VIEW_CHAMBER][USE_SELECT_CURRENT_PM_TYPE] );
				Set_Select_PMFILE( hdlg , MAX_VIEW_CHAMBER , "" );
			}
		}
		else {
			if ( PM_LOW_CONTROL[0] ) {
			...
			if ( PM_LOW_CONTROL[3] ) {
				FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM4FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 3 ) , PM_SUB_DIRECTORY[3][ PM_SUB_CURRENT_POS[3] ] , PM_LOW_DIRECTORY[3][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[3][USE_SELECT_CURRENT_PM_TYPE] );
				Set_Select_PMFILE( hdlg , 3 , "" );
			}
			if ( PM_LOW_CONTROL[4] ) {
				FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM5FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 4 ) , PM_SUB_DIRECTORY[4][ PM_SUB_CURRENT_POS[4] ] , PM_LOW_DIRECTORY[4][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[4][USE_SELECT_CURRENT_PM_TYPE] );
				Set_Select_PMFILE( hdlg , 4 , "" );
			}
			if ( PM_LOW_CONTROL[5] ) {
				FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM6FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 5 ) , PM_SUB_DIRECTORY[5][ PM_SUB_CURRENT_POS[5] ] , PM_LOW_DIRECTORY[5][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[5][USE_SELECT_CURRENT_PM_TYPE] );
				Set_Select_PMFILE( hdlg , 5 , "" );
			}
			if ( PM_LOW_CONTROL[6] ) {
				FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM7FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 6 ) , PM_SUB_DIRECTORY[6][ PM_SUB_CURRENT_POS[6] ] , PM_LOW_DIRECTORY[6][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[6][USE_SELECT_CURRENT_PM_TYPE] );
				Set_Select_PMFILE( hdlg , 6 , "" );
			}
			if ( PM_LOW_CONTROL[7] ) {
				FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM8FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 7 ) , PM_SUB_DIRECTORY[7][ PM_SUB_CURRENT_POS[7] ] , PM_LOW_DIRECTORY[7][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[7][USE_SELECT_CURRENT_PM_TYPE] );
				Set_Select_PMFILE( hdlg , 7 , "" );
			}
			if ( PM_LOW_CONTROL[8] ) {
				FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM9FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 8 ) , PM_SUB_DIRECTORY[8][ PM_SUB_CURRENT_POS[8] ] , PM_LOW_DIRECTORY[8][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[8][USE_SELECT_CURRENT_PM_TYPE] );
				Set_Select_PMFILE( hdlg , 8 , "" );
			}
			if ( PM_LOW_CONTROL[9] ) {
				FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM10FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 9 ) , PM_SUB_DIRECTORY[9][ PM_SUB_CURRENT_POS[9] ] , PM_LOW_DIRECTORY[9][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[9][USE_SELECT_CURRENT_PM_TYPE] );
				Set_Select_PMFILE( hdlg , 9 , "" );
			}
			if ( PM_LOW_CONTROL[10] ) {
				FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM11FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 10 ) , PM_SUB_DIRECTORY[10][ PM_SUB_CURRENT_POS[10] ] , PM_LOW_DIRECTORY[10][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[10][USE_SELECT_CURRENT_PM_TYPE] );
				Set_Select_PMFILE( hdlg , 10 , "" );
			}
			if ( PM_LOW_CONTROL[11] ) {
				FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM12FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 11 ) , PM_SUB_DIRECTORY[11][ PM_SUB_CURRENT_POS[11] ] , PM_LOW_DIRECTORY[11][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[11][USE_SELECT_CURRENT_PM_TYPE] );
				Set_Select_PMFILE( hdlg , 11 , "" );
			}
		}
	}
}
...
void PROCESS_SEND_BUTTON_MAPPING( HWND hdlg , int ch , int id , int Enable ) {
	...
	if ( Get_Prm_MAXVIEW_MODE() ) {
		switch ( ch ) {
		case 100 :
			index1 = IDB_INSERT_ENABLE;	index2 = IDB_INSERT_DISABLE;	index3 = IDB_INSERT_PRESSED;
			break;
		case 101 :
			index1 = IDB_REMOVE_ENABLE;	index2 = IDB_REMOVE_DISABLE;	index3 = IDB_REMOVE_PRESSED;
			break;
		case 102 :
		case 103 :
		...
		if ( ch == 100 ) {
			if ( Enable == 1 ) {
				Enable = 0;
				if ( Check_Select_PMFILE( hdlg , MAX_VIEW_CHAMBER ) ) {
					if ( MAX_VIEW_SELECT_POINTER >= 0 ) {
						if ( MAX_VIEW_SELECT_POINTER >= MAX_VIEW_START_POINTER ) {
							if ( MAX_VIEW_SELECT_POINTER < MAX_VIEW_START_POINTER + DISPLAY_MAX_VIEW_COUNT ) {
		...
		if ( ( ch == 102 ) || ( ch == 103 ) ) {
			if ( Enable == 1 ) {
				if ( MAX_VIEW_SELECT_POINTER >= 0 ) {
					switch( MAX_VIEW_TYPE ) {
					case 0 :
						KWIN_Item_Hide( hdlg , index1 );
						KWIN_Item_Enable( hdlg , index2 );
						KWIN_Item_Hide( hdlg , index3 );
		...
		if ( ch == 100 || ch == 101 ) {
			if ( Enable == 1 ) {
				...
				if ( ch == 100 ) {
					if ( !ClusterData_ListView_Enable( GetDlgItem( hdlg , IDC_LIST_CLUSTERDATA ) ) ) {
						KWIN_Item_Hide( hdlg , index1 );
						KWIN_Item_Hide( hdlg , index2 );
						KWIN_Item_Hide( hdlg , index3 );
						return;
				...
				if ( ch == 101 ) {
					if ( !ClusterData_ListView_Remove( GetDlgItem( hdlg , IDC_LIST_CLUSTERDATA ) ) ) {
						KWIN_Item_Hide( hdlg , index1 );
						KWIN_Item_Hide( hdlg , index2 );
						KWIN_Item_Hide( hdlg , index3 );
						return;
					...
					for ( j = 0 ; j < MAX_PM_CHAMBER_DEPTH ; j++ ) {
						if      ( Get_CLUSTER_USE_TYPE_ITEM() == 3 ) {
							if ( ( strlen( CLUSTER_RECIPE.MODULE_PR_RECIPE[i/Get_CLUSTER_USE_TYPE_ITEM()][j] ) > 0 ) || ( strlen( CLUSTER_RECIPE.MODULE_IN_RECIPE[i/Get_CLUSTER_USE_TYPE_ITEM()][j] ) > 0 ) || ( strlen( CLUSTER_RECIPE.MODULE_OUT_RECIPE[i/Get_CLUSTER_USE_TYPE_ITEM()][j] ) > 0 ) ) break;
						}
						else if ( Get_CLUSTER_USE_TYPE_ITEM() == 2 ) {
							if ( ( strlen( CLUSTER_RECIPE.MODULE_IN_RECIPE[i/Get_CLUSTER_USE_TYPE_ITEM()][j] ) > 0 ) || ( strlen( CLUSTER_RECIPE.MODULE_OUT_RECIPE[i/Get_CLUSTER_USE_TYPE_ITEM()][j] ) > 0 ) ) break;
						}
						else {
							if ( strlen( CLUSTER_RECIPE.MODULE_IN_RECIPE[i/Get_CLUSTER_USE_TYPE_ITEM()][j] ) > 0 ) break;
						}
					}
					if ( j != MAX_PM_CHAMBER_DEPTH ) {
						...
						case 3 :
							if ( ( i % 3 ) == 0 ) {
								for ( j = 0 ; j < MAX_PM_CHAMBER_DEPTH ; j++ ) {
									if ( strlen( CLUSTER_RECIPE.MODULE_PR_RECIPE[i/Get_CLUSTER_USE_TYPE_ITEM()][j] ) > 0 ) break;
								}
								if ( j == MAX_PM_CHAMBER_DEPTH ) {
									KWIN_Item_Hide( hdlg , index1 );
									KWIN_Item_Hide( hdlg , index2 );
								...
								for ( j = 0 ; j < MAX_PM_CHAMBER_DEPTH ; j++ ) {
									if ( strlen( CLUSTER_RECIPE.MODULE_IN_RECIPE[i/3][j] ) > 0 ) break;
								}
								if ( j == MAX_PM_CHAMBER_DEPTH ) {
									KWIN_Item_Hide( hdlg , index1 );
									KWIN_Item_Hide( hdlg , index2 );
								...
								for ( j = 0 ; j < MAX_PM_CHAMBER_DEPTH ; j++ ) {
									if ( strlen( CLUSTER_RECIPE.MODULE_OUT_RECIPE[i/3][j] ) > 0 ) break;
								}
								if ( j == MAX_PM_CHAMBER_DEPTH ) {
									KWIN_Item_Hide( hdlg , index1 );
									KWIN_Item_Hide( hdlg , index2 );
						...
						case 2 :
							if     ( ( i % 2 ) == 0 ) {
								for ( j = 0 ; j < MAX_PM_CHAMBER_DEPTH ; j++ ) {
									if ( strlen( CLUSTER_RECIPE.MODULE_IN_RECIPE[i/2][j] ) > 0 ) break;
								}
								if ( j == MAX_PM_CHAMBER_DEPTH ) {
									KWIN_Item_Hide( hdlg , index1 );
									KWIN_Item_Hide( hdlg , index2 );
								...
								for ( j = 0 ; j < MAX_PM_CHAMBER_DEPTH ; j++ ) {
									if ( strlen( CLUSTER_RECIPE.MODULE_OUT_RECIPE[i/2][j] ) > 0 ) break;
								}
								if ( j == MAX_PM_CHAMBER_DEPTH ) {
									KWIN_Item_Hide( hdlg , index1 );
									KWIN_Item_Hide( hdlg , index2 );
						...
						default :
							for ( j = 0 ; j < MAX_PM_CHAMBER_DEPTH ; j++ ) {
								if ( strlen( CLUSTER_RECIPE.MODULE_IN_RECIPE[i/1][j] ) > 0 ) break;
							}
							if ( j == MAX_PM_CHAMBER_DEPTH ) {
								KWIN_Item_Hide( hdlg , index1 );
								KWIN_Item_Hide( hdlg , index2 );
		...
		else if ( ch == 104 ) { // 2005.07.19
			if ( Get_Prm_ALLMOVE_MODE() ) {
				if ( Enable == 1 ) {
					if ( ClusterData_ListView_SelectCount( GetDlgItem( hdlg , IDC_LIST_CLUSTERDATA ) ) != 1 ) {
						KWIN_Item_Hide( hdlg , index1 );
						return;
					...
					for ( i = 0 ; i < MAX_PM_CHAMBER_DEPTH ; i++ ) {
						if ( Check_Select_PMFILE( hdlg , i ) ) break;
					}
					if ( i == MAX_PM_CHAMBER_DEPTH ) {
						...
						//------------------------------------------------------------------------------------------------------------------
						KWIN_Item_Resize( hdlg , index1 , P_XA[i][0] - P_ALL[i][0] - P_ALL[i][2] , P_YA[i][0] , P_ALL[i][0] , P_ALL[i][1] );
						KWIN_Item_Show( hdlg , index1 );
					}
				}
				else {
					KWIN_Item_Hide( hdlg , index1 );
...
void ClusterData_Button_Init( HWND hWnd , int mode , BOOL pmtypemode ) {
	if ( mode == 0 ) {
		PROCESS_SEND_BUTTON_MAPPING( hWnd , 100 , 0 , 0 );
		PROCESS_SEND_BUTTON_MAPPING( hWnd , 101 , 0 , 0 );
		if ( pmtypemode )
			PROCESS_SEND_BUTTON_MAPPING( hWnd , 103 , 0 , 0 );
		else
			PROCESS_SEND_BUTTON_MAPPING( hWnd , 102 , 0 , 0 );
		//
		PROCESS_SEND_BUTTON_MAPPING( hWnd , 104 , 0 , 0 ); // 2005.07.19
		//
		if ( Get_Prm_MAXVIEW_MODE() ) {
			PROCESS_SEND_BUTTON_MAPPING( hWnd , 99999 , 0 , 0 );
			PROCESS_SEND_BUTTON_MAPPING( hWnd , 99999 , 1 , 0 );
		}
		else {
		...
		PROCESS_SEND_BUTTON_MAPPING( hWnd , 100 , 0 , 1 );
		PROCESS_SEND_BUTTON_MAPPING( hWnd , 101 , 0 , 1 );
		if ( pmtypemode )
			PROCESS_SEND_BUTTON_MAPPING( hWnd , 103 , 0 , 1 );
		else
			PROCESS_SEND_BUTTON_MAPPING( hWnd , 102 , 0 , 1 );
		//
		PROCESS_SEND_BUTTON_MAPPING( hWnd , 104 , 0 , 1 ); // 2005.07.19
		//
		if ( Get_Prm_MAXVIEW_MODE() ) {
			PROCESS_SEND_BUTTON_MAPPING( hWnd , 99999 , 0 , 1 );
			PROCESS_SEND_BUTTON_MAPPING( hWnd , 99999 , 1 , 1 );
		}
		else {
...
BOOL ClusterData_ListView_Init( HWND hwndListView ) {
	...
	for ( i = 0 ; i < ( Get_Prm_DRAW_SIZE() + 1 ) ; i++ ) {
		switch( Get_Prm_DRAW_SIZE() ) {
		case 4 :
			if ( Get_Prm_MINISCREEN_MODE() ) lvColumn.cx = LV_Size_4_M[i];
			else                             lvColumn.cx = LV_Size_4[i];
			break;
		case 6 :
			if ( Get_Prm_MINISCREEN_MODE() ) lvColumn.cx = LV_Size_6_M[i];
			else                             lvColumn.cx = LV_Size_6[i];
			break;
...
void Set_CLUSTERDATA_DISPLAY( HWND hdlg , int mode ) {
	if ( Get_Prm_MAXVIEW_MODE() ) {
		if ( !mode ) {
			KWIN_Item_Hide( hdlg , IDC_CTRL_HANDLING_SIDE );
		}
		else {
...
BOOL Console_Cluster_File_Open( HWND hdlg , char *Buffer ) {
	...
	if ( Kor_CLUSTER_Open( localFilename , Get_Prm_GROUP_RECIPE_PATH() ) ) {
		...
		//
		Set_VERSION_INFO_REFRESH( hdlg , TRUE ); // 2007.09.18
		//
		...
		case WM_TIMER :
			...
			if ( tcount >= 15 ) {
				if ( Get_Prm_MAXVIEW_MODE() ) {
					...
					if ( KGUI_STANDARD_Window_Signal_Mode() ) { // 2004.05.21
						...
						SendMessage( hdlg , WM_COMMAND , IDC_CTRL_RELOAD_PM4 , (LPARAM) NULL );
						SendMessage( hdlg , WM_COMMAND , IDC_CTRL_RELOAD_PM5 , (LPARAM) NULL );
						SendMessage( hdlg , WM_COMMAND , IDC_CTRL_RELOAD_PM6 , (LPARAM) NULL );
						SendMessage( hdlg , WM_COMMAND , IDC_CTRL_RELOAD_PM7 , (LPARAM) NULL );
						SendMessage( hdlg , WM_COMMAND , IDC_CTRL_RELOAD_PM8 , (LPARAM) NULL );
						SendMessage( hdlg , WM_COMMAND , IDC_CTRL_RELOAD_PM9 , (LPARAM) NULL );
						SendMessage( hdlg , WM_COMMAND , IDC_CTRL_RELOAD_PM10 , (LPARAM) NULL );
						SendMessage( hdlg , WM_COMMAND , IDC_CTRL_RELOAD_PM11 , (LPARAM) NULL );
						SendMessage( hdlg , WM_COMMAND , IDC_CTRL_RELOAD_PM12 , (LPARAM) NULL );
						tcount = 0;
					}
					else {
						tcount--;
			...
			if ( Get_Prm_MONITOR_MODE() ) {
				...
				if ( ( i == 3 ) && ( oldcommsts != 3 ) ) {
					...
					//
					Set_VERSION_INFO_REFRESH( hdlg , FALSE ); // 2007.09.18
					//
					//------------------------------------------------------------------------------------------
					...
					if ( od2 != Get_Prm_MAXVIEW_MODE() ) { // 2004.04.06
						MAIN_WINDOW_REFRESH = TRUE;
						//===============================================
						FILE_HANDLING_BUTTON_MAKE( hdlg , 2 );
						//===============================================
					...
					if ( Get_Prm_MAXVIEW_MODE() ) {
						...
						ModuleData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PLFILE ) );
						FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PMFILE ) , Get_Prm_PROCESS_RECIPE_PATH( MAX_VIEW_CHAMBER ) , PM_SUB_DIRECTORY[MAX_VIEW_CHAMBER][ PM_SUB_CURRENT_POS[MAX_VIEW_CHAMBER] ] , PM_LOW_DIRECTORY[MAX_VIEW_CHAMBER][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[MAX_VIEW_CHAMBER][USE_SELECT_CURRENT_PM_TYPE] );
						if ( Get_Prm_MODULE_MODE( MAX_VIEW_CHAMBER ) )
							KWIN_Item_Enable( hdlg , IDC_LIST_PMFILE );
						else
							KWIN_Item_Disable( hdlg , IDC_LIST_PMFILE );
					}
					else {
						...
						KWIN_Item_String_Display( hdlg , IDC_CTRL_CHANGE_PM4 , PM_SUB_DIRECTORY[3][PM_SUB_CURRENT_POS[3]] );
						KWIN_Item_String_Display( hdlg , IDC_CTRL_CHANGE_PM5 , PM_SUB_DIRECTORY[4][PM_SUB_CURRENT_POS[4]] );
						KWIN_Item_String_Display( hdlg , IDC_CTRL_CHANGE_PM6 , PM_SUB_DIRECTORY[5][PM_SUB_CURRENT_POS[5]] );
						KWIN_Item_String_Display( hdlg , IDC_CTRL_CHANGE_PM7 , PM_SUB_DIRECTORY[6][PM_SUB_CURRENT_POS[6]] );
						KWIN_Item_String_Display( hdlg , IDC_CTRL_CHANGE_PM8 , PM_SUB_DIRECTORY[7][PM_SUB_CURRENT_POS[7]] );
						KWIN_Item_String_Display( hdlg , IDC_CTRL_CHANGE_PM9 , PM_SUB_DIRECTORY[8][PM_SUB_CURRENT_POS[8]] );
						KWIN_Item_String_Display( hdlg , IDC_CTRL_CHANGE_PM10 , PM_SUB_DIRECTORY[9][PM_SUB_CURRENT_POS[9]] );
						KWIN_Item_String_Display( hdlg , IDC_CTRL_CHANGE_PM11 , PM_SUB_DIRECTORY[10][PM_SUB_CURRENT_POS[10]] );
						KWIN_Item_String_Display( hdlg , IDC_CTRL_CHANGE_PM12 , PM_SUB_DIRECTORY[11][PM_SUB_CURRENT_POS[11]] );
						//===============================================
						FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_CLUSTERFILE ) , Get_Prm_CLUSTER_RECIPE_PATH() , "" , "" , "*.*" );
						FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM1FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 0 ) , PM_SUB_DIRECTORY[0][ PM_SUB_CURRENT_POS[0] ] , PM_LOW_DIRECTORY[0][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[0][USE_SELECT_CURRENT_PM_TYPE] );
						FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM2FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 1 ) , PM_SUB_DIRECTORY[1][ PM_SUB_CURRENT_POS[1] ] , PM_LOW_DIRECTORY[1][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[1][USE_SELECT_CURRENT_PM_TYPE] );
...
//				} // 2005.05.03
			...
			if ( Get_Prm_MAXVIEW_MODE() ) {
				...
				KWIN_Item_Hide( hdlg , IDC_LIST_CLUSTERDATA );
				//===============================================
				KWIN_Item_Enable( hdlg , IDC_LIST_PLFILE );
				//===============================================
//				if ( !Get_Prm_MONITOR_MODE() ) { // 2004.04.06 // 2005.05.03
					FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_CLUSTERFILE ) , Get_Prm_CLUSTER_RECIPE_PATH() , "" , "" , "*.*" );
					ModuleData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PLFILE ) );
...
//				} // 2005.05.03
				...
				KWIN_Item_Hide( hdlg , IDC_LIST_PLFILE );
				KWIN_Item_Hide( hdlg , IDC_LIST_PMFILE );
				//
				KWIN_Item_String_Display( hdlg , IDC_CTRL_CHANGE_PM1 , PM_SUB_DIRECTORY[0][PM_SUB_CURRENT_POS[0]] );
				KWIN_Item_String_Display( hdlg , IDC_CTRL_CHANGE_PM2 , PM_SUB_DIRECTORY[1][PM_SUB_CURRENT_POS[1]] );
				KWIN_Item_String_Display( hdlg , IDC_CTRL_CHANGE_PM3 , PM_SUB_DIRECTORY[2][PM_SUB_CURRENT_POS[2]] );
...
//				} // 2005.05.03
			...
			//
			if ( !Get_Prm_VERSION_MODE() ) { // 2007.09.18
				KWIN_Item_Hide( hdlg , IDC_CURRENT_LAST_USER );
				KWIN_Item_Hide( hdlg , IDC_CURRENT_LAST_LEVEL );
				KWIN_Item_Hide( hdlg , IDC_CURRENT_LAST_DATE );
			}
			else {
				if ( Get_Prm_TITLE_MODE() ) { // 2008.03.11
					SetWindowPos( GetDlgItem( hdlg , IDC_CURRENT_LAST_USER )  , NULL , 594 , 2 + 11 , 0 , 0 , SWP_NOSIZE | SWP_NOZORDER );
					SetWindowPos( GetDlgItem( hdlg , IDC_CURRENT_LAST_LEVEL ) , NULL , 689 , 2 + 11 , 0 , 0 , SWP_NOSIZE | SWP_NOZORDER );
					SetWindowPos( GetDlgItem( hdlg , IDC_CURRENT_LAST_DATE )  , NULL , 725 , 2 + 11 , 0 , 0 , SWP_NOSIZE | SWP_NOZORDER );
				}
				else {
					SetWindowPos( GetDlgItem( hdlg , IDC_CURRENT_LAST_USER )  , NULL , 594 , 2 , 0 , 0 , SWP_NOSIZE | SWP_NOZORDER );
					SetWindowPos( GetDlgItem( hdlg , IDC_CURRENT_LAST_LEVEL ) , NULL , 689 , 2 , 0 , 0 , SWP_NOSIZE | SWP_NOZORDER );
					SetWindowPos( GetDlgItem( hdlg , IDC_CURRENT_LAST_DATE )  , NULL , 725 , 2 , 0 , 0 , SWP_NOSIZE | SWP_NOZORDER );
				}
			}
...
//			if ( !KGUI_STANDARD_Window_Signal_Mode() ) return TRUE; // 2005.02.28
			...
			if ( !Get_Prm_MAXVIEW_MODE() ) {
				...
				// 2004.05.13
				//====================================================================================================
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_RECIPEDIR ) , NULL , FALSE );
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_CLUSTERFILE ) , NULL , FALSE );
				//
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_CLUSTERDATA ) , NULL , FALSE );
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_PM1FILE ) , NULL , FALSE );
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_PM2FILE ) , NULL , FALSE );
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_PM3FILE ) , NULL , FALSE );
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_PM4FILE ) , NULL , FALSE );
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_PM5FILE ) , NULL , FALSE );
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_PM6FILE ) , NULL , FALSE );
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_PM7FILE ) , NULL , FALSE );
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_PM8FILE ) , NULL , FALSE );
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_PM9FILE ) , NULL , FALSE );
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_PM10FILE ) , NULL , FALSE );
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_PM11FILE ) , NULL , FALSE );
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_PM12FILE ) , NULL , FALSE );
				//====================================================================================================
			}
			else {
				if ( KGUI_Get_BACKGROUND() == NULL ) {
				...
				//====================================================================================================
				ClusterData_MaxView_Draw( hdlg , ps.hdc );
				//====================================================================================================
				...
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_CLUSTERFILE ) , NULL , FALSE );
				//
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_PLFILE ) , NULL , FALSE );
				//InvalidateRect( GetDlgItem( hdlg , IDC_LIST_PMFILE ) , NULL , FALSE );
				//====================================================================================================
			}
			EndPaint( hdlg , &ps );
			return TRUE;
...
//			GoModalDialogBoxParam( GetModuleHandle(NULL) , MAKEINTRESOURCE( IDD_CLUSTER_PARAMETER ) , hdlg , Console_Gui_For_Cluster_Parameter_Proc , (LPARAM) NULL ); // 2004.01.19 // 2004.08.10
//			GoModalDialogBoxParam( GETHINST(hdlg) , MAKEINTRESOURCE( IDD_CLUSTER_PARAMETER ) , hdlg , Console_Gui_For_Cluster_Parameter_Proc , (LPARAM) NULL ); // 2004.01.19
//			GoModalDialogBoxParam( GetModuleHandle(NULL) , MAKEINTRESOURCE( IDD_CLUSTER_PARAMETER ) , hdlg , Console_Gui_For_Cluster_Parameter_Proc , (LPARAM) NULL );
			return TRUE;

		case WM_LBUTTONDOWN:
			if ( Get_Prm_MAXVIEW_MODE() ) {
				ClusterData_MaxView_LeftDown( hdlg );
			}
			return TRUE;
		...
		case WM_COMMAND :
			if ( GET_WM_COMMAND_CMD( wParam , lParam ) == LBN_DBLCLK ) {
				...
				case IDC_LIST_CLUSTERFILE :
					...
					break;
				case IDC_LIST_PLFILE :
					i = (int) ListBox_GetCurSel( GetDlgItem( hdlg , IDC_LIST_PLFILE ) );
					if ( ( i >= 0 ) && ( i != MAX_VIEW_CHAMBER ) ) {
//						MAX_VIEW_CHAMBER = i; // 2006.04.27
						MAX_VIEW_CHAMBER = ModuleData_ListView_Index( i ); // 2006.04.27
						Set_Select_PMFILE( hdlg , MAX_VIEW_CHAMBER , "" );
						FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PMFILE ) , Get_Prm_PROCESS_RECIPE_PATH( MAX_VIEW_CHAMBER ) , PM_SUB_DIRECTORY[MAX_VIEW_CHAMBER][ PM_SUB_CURRENT_POS[MAX_VIEW_CHAMBER] ] , PM_LOW_DIRECTORY[MAX_VIEW_CHAMBER][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[MAX_VIEW_CHAMBER][USE_SELECT_CURRENT_PM_TYPE] );
						if ( Get_Prm_MODULE_MODE( MAX_VIEW_CHAMBER ) )
							KWIN_Item_Enable( hdlg , IDC_LIST_PMFILE );
			...
			switch( wParam ) {
				case IDC_CTRL_RELOAD_CLUSTER :
					if ( Get_Prm_SEARCH_MODE() ) { // 2007.09.18
						//============================================================
						// 2007.09.17
						//============================================================
						if ( !GoModalDialogBoxParam( GETHINST( hdlg ) , MAKEINTRESOURCE( IDD_SEARCH_PANEL ) , hdlg , SEARCH_PANEL_Proc , (LPARAM) NULL ) ) return TRUE;
						//============================================================
					}
					FileData_ListView_Init2( GetDlgItem( hdlg , IDC_LIST_CLUSTERFILE ) , Get_Prm_CLUSTER_RECIPE_PATH() , "" , "" , "*.*" );
					return TRUE;

				case IDC_CTRL_HANDLING_SIDE :
				...
				case IDB_SELECT_RCP_MAIN :
					if ( Get_Prm_MAXVIEW_MODE() ) {
						...
						case 3 :
							MAX_VIEW_TYPE = 2;
							PROCESS_SEND_BUTTON_MAPPING( hdlg , 102 , 0 , 1 );
							break;
						case 2 :
							MAX_VIEW_TYPE = 1;
							PROCESS_SEND_BUTTON_MAPPING( hdlg , 102 , 0 , 1 );
				...
				case IDB_INSERT_ENABLE :
					...
					PROCESS_SEND_BUTTON_MAPPING( hdlg , 100 , 0 , 2 );
					if ( Get_Prm_MAXVIEW_MODE() ) {
						i = ClusterData_MaxView_Match_Sel_Chamber( hdlg , &selcl ); // 2002.05.20
						//1=use chamber
						//2=last
						//0=unuse chamber
						//3=full
						switch( i ) {
						case 0 :
							if ( !CHAMBER_EXCLUSIVE_ENABLE( MAX_VIEW_CHAMBER , -1 ) ) { // 2002.05.20
								MessageBox( hdlg , "CHAMBER SELECT IMPOSSIBLE(Interlock)" , "ERROR", MB_ICONHAND | MB_OK );
								PROCESS_SEND_BUTTON_MAPPING( hdlg , 100 , 0 , 1 );
							}
							else {
								if ( CHAMBER_EXCLUSIVE_ENABLE( MAX_VIEW_CHAMBER , selcl ) ) { // 2002.05.20
									j = -1;
									if ( MODAL_DIGITAL_BOX1( hdlg , "Select" , "Select Insert Mode" , "Insert at Same Order|Insert New Order", &j ) ) {
										if ( ClusterData_MaxView_Match_Sel_Order( hdlg , &k , &l ) ) {
											Set_CLUSTER_FILE_CHANGE( TRUE );
											//
											Set_Enable_CLUSTERFILE( hdlg , FALSE );
											//
											...
											FILE_HANDLING_BUTTON_MAPPING( hdlg , BUTTON_FILE , 0 );
											//
											ClusterData_Insert_for_MaxView( hdlg , k , MAX_VIEW_TYPE , j );
											//
											ClusterData_Get_Last_View( FALSE );
											InvalidateRect( hdlg , NULL , FALSE );
											ClusterData_Button_Set( hdlg , FALSE );
										}
										else {
									...
									j = -1;
									if ( MODAL_DIGITAL_BOX1( hdlg , "Select" , "Select Insert Mode" , "Insert New Order", &j ) ) {
										if ( ClusterData_MaxView_Match_Sel_Order( hdlg , &k , &l ) ) {
											Set_CLUSTER_FILE_CHANGE( TRUE );
											//
											Set_Enable_CLUSTERFILE( hdlg , FALSE );
											...
											FILE_HANDLING_BUTTON_MAPPING( hdlg , BUTTON_FILE , 0 );
											//
											ClusterData_Insert_for_MaxView( hdlg , k , MAX_VIEW_TYPE , 1 );
											//
											ClusterData_Get_Last_View( FALSE );
											InvalidateRect( hdlg , NULL , FALSE );
											ClusterData_Button_Set( hdlg , FALSE );
						...
						case 1 :
							if ( CHAMBER_EXCLUSIVE_ENABLE( MAX_VIEW_CHAMBER , -1 ) ) { // 2002.05.20
								j = -1;
								if ( MODAL_DIGITAL_BOX1( hdlg , "Select" , "Select Insert Mode" , "Replace at Same Chamber Order|Insert New Order", &j ) ) {
									if ( ClusterData_MaxView_Match_Sel_Order( hdlg , &k , &l ) ) {
										Set_CLUSTER_FILE_CHANGE( TRUE );
										//
										Set_Enable_CLUSTERFILE( hdlg , FALSE );
								...
								j = -1;
								if ( MODAL_DIGITAL_BOX1( hdlg , "Select" , "Select Insert Mode" , "Replace at Same Chamber Order", &j ) ) {
									if ( ClusterData_MaxView_Match_Sel_Order( hdlg , &k , &l ) ) {
										Set_CLUSTER_FILE_CHANGE( TRUE );
										//
										Set_Enable_CLUSTERFILE( hdlg , FALSE );
						...
						case 2 :
							...
							break;
						case 3 :
							MessageBox( hdlg , "CHAMBER SELECT FULL" , "ERROR", MB_ICONHAND | MB_OK );
							PROCESS_SEND_BUTTON_MAPPING( hdlg , 100 , 0 , 1 );
							break;
						}
					}
						...
						if ( i >= 0 ) {
							...
							FILE_HANDLING_BUTTON_MAPPING( hdlg , BUTTON_FILE , 0 );
							//
							ClusterData_Insert( i / Get_CLUSTER_USE_TYPE_ITEM() );
							//
							InvalidateRect( GetDlgItem( hdlg , IDC_LIST_CLUSTERDATA ) , NULL , FALSE );
							ClusterData_Button_Set( hdlg , FALSE );
						}
				...
				case IDB_REMOVE_ENABLE :
					...
					PROCESS_SEND_BUTTON_MAPPING( hdlg , 101 , 0 , 2 );
					if ( Get_Prm_MAXVIEW_MODE() ) {
						if ( ClusterData_MaxView_Delete( hdlg ) ) {
							Set_CLUSTER_FILE_CHANGE( TRUE );
							//
							Set_Enable_CLUSTERFILE( hdlg , FALSE );
							//
							...
							PROCESS_SEND_BUTTON_MAPPING( hdlg , 101 , 0 , 1 );
						}
					}
					else {
						i = ClusterData_ListView_SelectData( GetDlgItem( hdlg , IDC_LIST_CLUSTERDATA ) , -1 );
						if ( i >= 0 ) {
							...
							while ( i >= 0 ) {
								...
								case 3 :
									if ( ( i % 3 ) == 0 ) {
										for ( j = 0 ; j < MAX_PM_CHAMBER_DEPTH ; j++ ) strcpy( CLUSTER_RECIPE.MODULE_PR_RECIPE[i/3][j] , "" );
									}
									else if ( ( i % 3 ) == 1 ) {
										for ( j = 0 ; j < MAX_PM_CHAMBER_DEPTH ; j++ ) strcpy( CLUSTER_RECIPE.MODULE_IN_RECIPE[i/3][j] , "" );
									}
									else {
										for ( j = 0 ; j < MAX_PM_CHAMBER_DEPTH ; j++ ) strcpy( CLUSTER_RECIPE.MODULE_OUT_RECIPE[i/3][j] , "" );
									}
									break;
								case 2 :
									if ( ( i % 2 ) == 0 ) {
										for ( j = 0 ; j < MAX_PM_CHAMBER_DEPTH ; j++ ) strcpy( CLUSTER_RECIPE.MODULE_IN_RECIPE[i/2][j] , "" );
									}
									else {
										for ( j = 0 ; j < MAX_PM_CHAMBER_DEPTH ; j++ ) strcpy( CLUSTER_RECIPE.MODULE_OUT_RECIPE[i/2][j] , "" );
									}
									break;
								default :
									for ( j = 0 ; j < MAX_PM_CHAMBER_DEPTH ; j++ ) strcpy( CLUSTER_RECIPE.MODULE_IN_RECIPE[i/1][j] , "" );
									break;
								}
								i = ClusterData_ListView_SelectData( GetDlgItem( hdlg , IDC_LIST_CLUSTERDATA ) , i );
							}
				...
				//==============================================================================
				//==============================================================================
				case IDC_CTRL_CHANGE_ALL : // 2005.07.19
					if ( Get_Prm_READONLY_MODE() || SYSTEM_LOGSKIP_STATUS() ) {
						MessageBox( hdlg , "Operation Impossible(Read Only Mode)" , "Operation Impossible" , MB_ICONHAND );
						return TRUE;
					}
					if ( !Get_Prm_MAXVIEW_MODE() ) {
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE_P1 ) ) && ( Check_Select_PMFILE( hdlg , 0 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE_P1 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE_P2 ) ) && ( Check_Select_PMFILE( hdlg , 1 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE_P2 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE_P3 ) ) && ( Check_Select_PMFILE( hdlg , 2 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE_P3 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE_P4 ) ) && ( Check_Select_PMFILE( hdlg , 3 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE_P4 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE_P5 ) ) && ( Check_Select_PMFILE( hdlg , 4 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE_P5 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE_P6 ) ) && ( Check_Select_PMFILE( hdlg , 5 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE_P6 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE_P7 ) ) && ( Check_Select_PMFILE( hdlg , 6 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE_P7 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE_P8 ) ) && ( Check_Select_PMFILE( hdlg , 7 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE_P8 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE_P9 ) ) && ( Check_Select_PMFILE( hdlg , 8 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE_P9 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE_P10 ) ) && ( Check_Select_PMFILE( hdlg , 9 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE_P10 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE_P11 ) ) && ( Check_Select_PMFILE( hdlg , 10 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE_P11 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE_P12 ) ) && ( Check_Select_PMFILE( hdlg , 11 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE_P12 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE2_P1 ) ) && ( Check_Select_PMFILE( hdlg , 0 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE2_P1 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE2_P2 ) ) && ( Check_Select_PMFILE( hdlg , 1 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE2_P2 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE2_P3 ) ) && ( Check_Select_PMFILE( hdlg , 2 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE2_P3 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE2_P4 ) ) && ( Check_Select_PMFILE( hdlg , 3 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE2_P4 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE2_P5 ) ) && ( Check_Select_PMFILE( hdlg , 4 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE2_P5 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE2_P6 ) ) && ( Check_Select_PMFILE( hdlg , 5 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE2_P6 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE2_P7 ) ) && ( Check_Select_PMFILE( hdlg , 6 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE2_P7 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE2_P8 ) ) && ( Check_Select_PMFILE( hdlg , 7 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE2_P8 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE2_P9 ) ) && ( Check_Select_PMFILE( hdlg , 8 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE2_P9 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE2_P10 ) ) && ( Check_Select_PMFILE( hdlg , 9 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE2_P10 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE2_P11 ) ) && ( Check_Select_PMFILE( hdlg , 10 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE2_P11 , (LPARAM) NULL );
						}
						if ( IsWindowEnabled( GetDlgItem( hdlg , IDB_ADD_ENABLE2_P12 ) ) && ( Check_Select_PMFILE( hdlg , 11 ) ) ) {
							SendMessage( hdlg , WM_COMMAND , IDB_ADD_ENABLE2_P12 , (LPARAM) NULL );
						}
					}
					return TRUE;
				//==============================================================================
				...
				case IDC_CTRL_CHANGE_PM4 :
				case IDC_CTRL_CHANGE_PM5 :
				case IDC_CTRL_CHANGE_PM6 :
				case IDC_CTRL_CHANGE_PM7 :
				case IDC_CTRL_CHANGE_PM8 :
				case IDC_CTRL_CHANGE_PM9 :
				case IDC_CTRL_CHANGE_PM10 :
				case IDC_CTRL_CHANGE_PM11 :
				case IDC_CTRL_CHANGE_PM12 :
					if      ( wParam == IDC_CTRL_CHANGE_PM1 ) j = 0;
					else if ( wParam == IDC_CTRL_CHANGE_PM2 ) j = 1;
					else if ( wParam == IDC_CTRL_CHANGE_PM3 ) j = 2;
					else if ( wParam == IDC_CTRL_CHANGE_PM4 ) j = 3;
					else if ( wParam == IDC_CTRL_CHANGE_PM5 ) j = 4;
					else if ( wParam == IDC_CTRL_CHANGE_PM6 ) j = 5;
					else if ( wParam == IDC_CTRL_CHANGE_PM7 ) j = 6;
					else if ( wParam == IDC_CTRL_CHANGE_PM8 ) j = 7;
					else if ( wParam == IDC_CTRL_CHANGE_PM9 ) j = 8;
					else if ( wParam == IDC_CTRL_CHANGE_PM10 ) j = 9;
					else if ( wParam == IDC_CTRL_CHANGE_PM11 ) j = 10;
					else if ( wParam == IDC_CTRL_CHANGE_PM12 ) j = 11;
					...
					if ( MODAL_DIGITAL_BOX1( hdlg , "Select" , "Select Mode" , Buffer , &i ) ) {
						...
						if ( j ==  3 ) FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM4FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 3 ) , PM_SUB_DIRECTORY[3][ PM_SUB_CURRENT_POS[3] ] , PM_LOW_DIRECTORY[3][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[3][USE_SELECT_CURRENT_PM_TYPE] );
						if ( j ==  4 ) FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM5FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 4 ) , PM_SUB_DIRECTORY[4][ PM_SUB_CURRENT_POS[4] ] , PM_LOW_DIRECTORY[4][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[4][USE_SELECT_CURRENT_PM_TYPE] );
						if ( j ==  5 ) FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM6FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 5 ) , PM_SUB_DIRECTORY[5][ PM_SUB_CURRENT_POS[5] ] , PM_LOW_DIRECTORY[5][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[5][USE_SELECT_CURRENT_PM_TYPE] );
						if ( j ==  6 ) FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM7FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 6 ) , PM_SUB_DIRECTORY[6][ PM_SUB_CURRENT_POS[6] ] , PM_LOW_DIRECTORY[6][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[6][USE_SELECT_CURRENT_PM_TYPE] );
						if ( j ==  7 ) FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM8FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 7 ) , PM_SUB_DIRECTORY[7][ PM_SUB_CURRENT_POS[7] ] , PM_LOW_DIRECTORY[7][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[7][USE_SELECT_CURRENT_PM_TYPE] );
						if ( j ==  8 ) FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM9FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 8 ) , PM_SUB_DIRECTORY[8][ PM_SUB_CURRENT_POS[8] ] , PM_LOW_DIRECTORY[8][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[8][USE_SELECT_CURRENT_PM_TYPE] );
						if ( j ==  9 ) FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM10FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 9 ) , PM_SUB_DIRECTORY[9][ PM_SUB_CURRENT_POS[9] ] , PM_LOW_DIRECTORY[9][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[9][USE_SELECT_CURRENT_PM_TYPE] );
						if ( j == 10 ) FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM11FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 10 ) , PM_SUB_DIRECTORY[10][ PM_SUB_CURRENT_POS[10] ] , PM_LOW_DIRECTORY[10][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[10][USE_SELECT_CURRENT_PM_TYPE] );
						if ( j == 11 ) FileData_ListView_Init( GetDlgItem( hdlg , IDC_LIST_PM12FILE ) , Get_Prm_PROCESS_RECIPE_PATH( 11 ) , PM_SUB_DIRECTORY[11][ PM_SUB_CURRENT_POS[11] ] , PM_LOW_DIRECTORY[11][USE_SELECT_CURRENT_PM_TYPE] , PM_LOW_EXTENSION[11][USE_SELECT_CURRENT_PM_TYPE] );
...
//				GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_M , IDD_E_MAIN_CONTROL_M ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19 // 2004.08.10
	//			GoModalDialogBoxParam( GETHINST(hWnd) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_M , IDD_E_MAIN_CONTROL_M ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19
	//			GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_M , IDD_E_MAIN_CONTROL_M ) , GET_MAIN_WINDOW() , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2002.06.17
			}
			else {
...
//				GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL , IDD_E_MAIN_CONTROL ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19 // 2004.08.10
	//			GoModalDialogBoxParam( GETHINST(hWnd) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL , IDD_E_MAIN_CONTROL ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19
	//			GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL , IDD_E_MAIN_CONTROL ) , GET_MAIN_WINDOW() , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2002.06.17
			}
		}
		else {
			switch( Get_Prm_DRAW_SIZE() ) {
...
//					GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL , IDD_E_MAIN_CONTROL ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19 // 2004.08.10
	...
	//				GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL , IDD_E_MAIN_CONTROL ) , GET_MAIN_WINDOW() , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2002.06.17
			...
			case 6 :
				if ( Get_Prm_MINISCREEN_MODE() ) {
					GoModalDialogBoxParam( GETHINST(hWnd) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_6_M , IDD_E_MAIN_CONTROL_6_M ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19 // 2004.08.10
//					GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_6_M , IDD_E_MAIN_CONTROL_6_M ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19 // 2004.08.10
	//				GoModalDialogBoxParam( GETHINST(hWnd) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_6_M , IDD_E_MAIN_CONTROL_6_M ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19
	//				GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_6_M , IDD_E_MAIN_CONTROL_6_M ) , GET_MAIN_WINDOW() , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2002.06.17
				}
				else {
					GoModalDialogBoxParam( GETHINST(hWnd) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_6 , IDD_E_MAIN_CONTROL_6 ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19 // 2004.08.10
//					GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_6 , IDD_E_MAIN_CONTROL_6 ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19 // 2004.08.10
	//				GoModalDialogBoxParam( GETHINST(hWnd) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_6 , IDD_E_MAIN_CONTROL_6 ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19
	//				GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_6 , IDD_E_MAIN_CONTROL_6 ) , GET_MAIN_WINDOW() , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2002.06.17
				}
				break;
			case 8 :
				GoModalDialogBoxParam( GETHINST(hWnd) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_8 , IDD_E_MAIN_CONTROL_8 ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19 // 2004.08.10
//				GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_8 , IDD_E_MAIN_CONTROL_8 ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19 // 2004.08.10
	//			GoModalDialogBoxParam( GETHINST(hWnd) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_8 , IDD_E_MAIN_CONTROL_8 ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19
	//			GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_8 , IDD_E_MAIN_CONTROL_8 ) , GET_MAIN_WINDOW() , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2002.06.17
				break;
			case 10 :
				GoModalDialogBoxParam( GETHINST(hWnd) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_10 , IDD_E_MAIN_CONTROL_10 ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19 // 2004.08.10
//				GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_10 , IDD_E_MAIN_CONTROL_10 ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19 // 2004.08.10
	//			GoModalDialogBoxParam( GETHINST(hWnd) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_10 , IDD_E_MAIN_CONTROL_10 ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19
	//			GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_10 , IDD_E_MAIN_CONTROL_10 ) , GET_MAIN_WINDOW() , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2002.06.17
				break;
			case 12 :
				GoModalDialogBoxParam( GETHINST(hWnd) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_12 , IDD_E_MAIN_CONTROL_12 ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19 // 2004.08.10
//				GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_12 , IDD_E_MAIN_CONTROL_12 ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19 // 2004.08.10
	//			GoModalDialogBoxParam( GETHINST(hWnd) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_12 , IDD_E_MAIN_CONTROL_12 ) , hWnd , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2004.01.19
	//			GoModalDialogBoxParam( GetModuleHandle(NULL) , KWIN_MAKEINTRESOURCE( IDD_MAIN_CONTROL_12 , IDD_E_MAIN_CONTROL_12 ) , GET_MAIN_WINDOW() , Console_Gui_For_Main_Proc , (LPARAM) NULL ); // 2002.06.17
				break;
			}
		}
	}
...
void Program_Enter_Code() {
	...
	Set_Prm_MAXVIEW_MODE( FALSE );
	Set_Prm_TITLE_MODE( FALSE );
	Set_Prm_READONLY_LEVEL( 0 );
	Set_Prm_GROUPBUTTONHIDE_MODE( FALSE );
	Set_Prm_NAME_STYLE( 0 ); // 2004.06.15
	Set_Prm_ALLMOVE_MODE( FALSE );
	Set_Prm_NAMEONLY_MODE( FALSE ); // 2006.04.27
	Set_Prm_SEARCH_MODE( FALSE ); // 2007.09.18
	Set_Prm_VERSION_MODE( FALSE ); // 2007.09.18
	SAVE_PRESET_MODE = FALSE; // 2008.02.14
	//
	for ( i = 0 ; i < (signed) strlen( NAME2 ) ; i++ ) {
//		if ( tolower( NAME2[i] ) == 'd' ) DISPLAY_MODE = TRUE; // next
		if ( tolower( NAME2[i] ) == 'g' ) Set_GROUP_RUN_CHECK( TRUE );
		if ( tolower( NAME2[i] ) == 'm' ) Set_Prm_MINISCREEN_MODE( TRUE );
		if ( tolower( NAME2[i] ) == 'x' ) Set_Prm_MAXVIEW_MODE( TRUE );
		if ( tolower( NAME2[i] ) == 't' ) Set_Prm_TITLE_MODE( TRUE );
		if ( tolower( NAME2[i] ) == 'u' ) Set_Prm_NAME_STYLE( 1 ); // 2004.06.15
		if ( tolower( NAME2[i] ) == 'l' ) Set_Prm_NAME_STYLE( 2 ); // 2004.06.15
		if ( tolower( NAME2[i] ) == 'a' ) Set_Prm_ALLMOVE_MODE( TRUE ); // 2005.07.19
		if ( tolower( NAME2[i] ) == 'n' ) Set_Prm_NAMEONLY_MODE( TRUE ); // 2006.04.27
		...
		if ( tolower( NAME2[i] ) == 'h' ) Set_Prm_GROUPBUTTONHIDE_MODE( TRUE );
		if ( tolower( NAME2[i] ) == 's' ) Set_Prm_SEARCH_MODE( TRUE ); // 2007.09.18
		if ( tolower( NAME2[i] ) == 'v' ) Set_Prm_VERSION_MODE( TRUE ); // 2007.09.18
	...
	//
	if ( Get_Prm_TITLE_MODE() ) {
		DISPLAY_MAX_VIEW_COUNT = 5;
		DISPLAY_MAX_VIEW_START_Y = 70 + 40;
		//
		DISPLAY_MAX_VIEW_UP2_Y	=	85 + 40;
		DISPLAY_MAX_VIEW_UP_Y	=	160;
		DISPLAY_MAX_VIEW_DN_Y	=	195;
		DISPLAY_MAX_VIEW_DN2_Y	=	230;
	}
	else {
