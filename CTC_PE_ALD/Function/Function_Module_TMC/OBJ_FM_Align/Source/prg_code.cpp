...
IO_Name_String_Map IO_STR_Map_Table[] = {
	...
	{ "FM.Robot_Post_Pos"		, _K_A_IO	,	POSITION			,	0	} ,
	...
	//
	{ "FM.Robot_Run_Status"		, _K_D_IO	,	ATR8_Run_Status		,	0	} ,	
...
{
//------------------------------------------------------------------------------------------
Module_Status WaitRobotBusyCheck()
{
	int nTimeoutCount    = 0;
	int nMaxTimeoutCount = 20;
    int Data,CommStatus;
	...
	do{
		...
		nTimeoutCount++;
		Data = READ_DIGITAL( ATR8_Run_Status , &CommStatus );
		if(	Data == 0 /* Not Busy */ )
		{
			return SYS_SUCCESS;
		}
		...
		{
			if(nTimeoutCount > nMaxTimeoutCount)
			{
				ALARM_POST( ALARM_ALIGNER_SET_POST_POSITION );
				return SYS_ABORTED;
			}
			else
			{
...
Module_Status Program_Main() {
	...
	if ( SIMULATION_MODE )
	{
		_LOG("-----> '%s' Started in FM Aligner Func(Sim)", pParam);
		...
		// 2005.12.09
		sprintf(szCmd,"ALIGNER %s", RunStr);
		if(SYS_SUCCESS != g_DynIO.dRUN_FUNCTION(TMSIM_ALIGNER, szCmd)) msRet = SYS_ABORTED;

		_LOG("-----> '%s' Finished in FM Aligner Func(Sim)", pParam);
