...
void CPersistIO::DoMonitor()
{
	...
	while(WaitSec(1))
	{
		for(i=0; i<MAX_IO_LIST && m_pIOList[i].szIOName[0]!=0; i++)
		{
			...
			if(strcmp(szValue, m_pIOValue[i]) != 0)
			{
...
bool CPersistIO::Initialize(char *szConfigPath)
{
	...
	if(! bRet) printf("-----> Initalization failed in PersistIO\n");
	return bRet;
}
...
bool CPersistIO::WriteIOValueToFile()
{
	...
	do {
		...
		for(i=0; i<MAX_IO_LIST && m_pIOList[i].szIOName[0]!=0; i++)
		{
			...
			switch(m_pIOList[i].nIOType)
			{
			case _K_D_IO: sprintf(szTemp, "%s	D	%s\n", m_pIOList[i].szIOName, m_pIOValue[i]); break;
			case _K_A_IO: sprintf(szTemp, "%s	A	%s\n", m_pIOList[i].szIOName, m_pIOValue[i]); break;
			case _K_S_IO: sprintf(szTemp, "%s	S	%s\n", m_pIOList[i].szIOName, m_pIOValue[i]); break;
			}
			fputs(szTemp, fp);
		}
		bRet = true;
