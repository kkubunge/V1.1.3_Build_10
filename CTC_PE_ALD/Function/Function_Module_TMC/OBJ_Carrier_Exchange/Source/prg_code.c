...
#define	ALARM_PM_RUN_STATUS_ERROR		183	// jelee added. 2007.12.06
...
//	{ "$1.AGV_MODE_STS"				, _K_D_IO	,	LP_MODE_AGV_STS		,	0	} ,
...
//	{ "$0.UnClamp"					, _K_D_IO	,	UNCLAMP_SET			,	0	} ,
//	{ "$0.Clamp"					, _K_D_IO	,	CLAMP_SET			,	0	} ,
...
//				LOG(pszFtnName, ELT_File, "======> 12 [LPM%d] AGV_RECOVERY_SCENARIO UNLOAD case [CarrierSta = PRESENT]", CASSETTE_INDEX);
				{
					...
					if ( AlmResult == ALM_IGNORE ) {
						...
						if ( Status != SYS_SUCCESS ) {
							...
							sprintf( StrBuffer , EVENT_STRING_TR_UNLOAD_RECV , CASSETTE_INDEX );    //2014.06.25
							WRITE_STRING( SCHEDULER , StrBuffer , &CommStatus );
							//-------------------------------------------------------------------------------
							return SYS_SUCCESS;
						}
...
Module_Status READ_DIGITAL_TRUE( int io , int *Data ) {
	...
	while ( TRUE ) {
		//... 2015.11.23 Add 15 Comm Spec (OHT Unload/Load Interlock)
		//for( i = 0 ; i < 3 ; i++ ) {
		for( i = 0 ; i < 2 ; i++ ) {
...
//			LOG(pszFtnName, ELT_File, "[LPM%d] READ DIGITAL TRUE [Retry = %d]", CASSETTE_INDEX, i);
			*Data = READ_DIGITAL( io , &CommStatus );
			if ( !CommStatus ) {
				...
				if ( !WAIT_SECONDS( (double)WAIT_TIME ) ) return SYS_ABORTED;
			}
			else 
				break;
		}
...
Module_Status AGV_LOAD_CONTROL() {
	...
	while( TRUE ) {
		...
		//... 2015.11.23 Add 15 Comm Spec (OHT Unload/Load Interlock)
		// 2015.11.23 Abnormal Case Alarm : TD0
		if ( READ_DIGITAL( LP_AGV_INPUT_CS , &CommStatus ) == OFF		|| READ_DIGITAL( LP_AGV_INPUT_TRREQ , &CommStatus ) == ON || 
			 READ_DIGITAL( LP_AGV_INPUT_BUSY , &CommStatus ) == ON		|| READ_DIGITAL( LP_AGV_INPUT_COMPLETE , &CommStatus ) == ON ){
...
//	}
	...
	while( TRUE ) {
		...
		//... 2015.11.23 Add 15 Comm Spec (OHT Unload/Load Interlock)
		SIG_VALID = READ_DIGITAL( LP_AGV_INPUT_VALID , &CommStatus ) ;
		// 2015.11.23 Abnormal Case Alarm : TP1		
		//if ( READ_DIGITAL( LP_AGV_INPUT_VALID , &CommStatus ) == OFF ) {		
		if ( READ_DIGITAL( LP_AGV_INPUT_CS , &CommStatus ) == OFF	|| SIG_VALID == OFF || 
			 READ_DIGITAL( LP_AGV_INPUT_BUSY , &CommStatus ) == ON  || READ_DIGITAL( LP_AGV_INPUT_COMPLETE , &CommStatus ) == ON ){
...
//			AGV_INIT_SET_IO( -2 ); // recovery is only ignore and abbort
			...
			}else AlmResult = ALARM_MANAGE_for_CarrHandling( ALARM_START_POINTER + ALARM_SENARIO_ERROR );
			if ( AlmResult != SYS_SUCCESS ) { // Retry Option
//				LOG(pszFtnName, ELT_File, "[LPM%d] ALARM_VALIDOFF_ERROR When Load Sequence------>1-1", CASSETTE_INDEX);
				Status = AGV_RECOVERY_SCENARIO3( LOAD, RETRY1 );
				if ( Status != SYS_SUCCESS ) { // Retry Option
...
//			AGV_INIT_SET_IO( -2 ); // recovery is only ignore and abbort
	...
	while( TRUE ) {
		...
		//... 2015.11.23 Add 15 Comm Spec (OHT Unload/Load Interlock)
		SIG_VALID = READ_DIGITAL( LP_AGV_INPUT_VALID , &CommStatus ) ;
		// 2015.11.23 Abnormal Case Alarm : TP2		
		//if ( READ_DIGITAL( LP_AGV_INPUT_VALID , &CommStatus ) == OFF ) {
		if ( READ_DIGITAL( LP_AGV_INPUT_CS , &CommStatus ) == OFF		|| READ_DIGITAL( LP_AGV_INPUT_VALID , &CommStatus ) == OFF || 
			 READ_DIGITAL( LP_AGV_INPUT_TRREQ , &CommStatus ) == OFF	|| READ_DIGITAL( LP_AGV_INPUT_COMPLETE , &CommStatus ) == ON ){
...
//			AGV_INIT_SET_IO( -2 ); // recovery is only ignore and abbort
	...
	while ( TRUE ) {
		...
		//... 2015.11.23 Add 15 Comm Spec (OHT Unload/Load Interlock)
		SIG_VALID = READ_DIGITAL( LP_AGV_INPUT_VALID , &CommStatus ) ;
		// 2015.11.23 Abnormal Case Alarm : TP3	
		//if ( READ_DIGITAL( LP_AGV_INPUT_VALID , &CommStatus ) == OFF ) {
		if ( READ_DIGITAL( LP_AGV_INPUT_CS , &CommStatus ) == OFF	  || SIG_VALID == OFF   || 
		  	 READ_DIGITAL( LP_AGV_INPUT_TRREQ , &CommStatus ) == OFF  || READ_DIGITAL( LP_AGV_INPUT_COMPLETE , &CommStatus ) == ON ||
			 READ_DIGITAL( LP_AGV_INPUT_BUSY  , &CommStatus ) == OFF  ){
...
//			AGV_INIT_SET_IO( -2 ); // recovery is only ignore and abbort
	...
	while( TRUE ) {
		...
		//... 2015.11.23 Add 15 Comm Spec (OHT Unload/Load Interlock)
		// 2015.11.23 Abnormal Case Alarm : TP4		
		//if ( READ_DIGITAL( LP_AGV_INPUT_VALID , &CommStatus ) == OFF ) {
		if ( READ_DIGITAL( LP_AGV_INPUT_CS , &CommStatus ) == OFF || READ_DIGITAL( LP_AGV_INPUT_VALID , &CommStatus ) == OFF){
...
//					}
	...
	return SYS_SUCCESS;
/* jelee Delete Start 2005.08.08
	int AlmResult , CommStatus , AGV_Sts , Error , Alarm , Data;
	BOOL Event = TRUE;

	if ( SIMULATION_AGV_MODE ) WRITE_DIGITAL( LP_AGV_STS , BUSY , &CommStatus );
	while( TRUE ) {
		_sleep(250);

		switch( READ_DIGITAL( LP_MDL_STATUS , &CommStatus ) ) {
			case 1 : case 4 : break;
			default: return SYS_ABORTED;
		}
		
		if ( READ_DIGITAL_TRUE( LP_AGV_STS , &AGV_Sts ) == SYS_ABORTED ) return SYS_ABORTED;
		if ( AGV_Sts == ERROR1 ) { 
			Alarm = ALARM_PORT_AGV_ERROR;
			if ( ALARM_MESSAGE_REMAPPING( ALARM_START_POINTER + ALARM_PORT_AGV_ERROR , &Error ) ) {
				if		( Error == 651 ) Alarm = ALARM_TRREQ_TIMEOUT_TP1;    // T1
				else if ( Error == 652 ) Alarm = ALARM_BUSY_TIMEOUT_TP2;     // T2
				else if ( Error == 653 ) Alarm = ALARM_FOUPRECV_TIMEOUT_TP3; // T3
				else if ( Error == 654 ) Alarm = ALARM_COMPLETE_TIMEOUT_TP4; // T4
				else if ( Error == 655 ) Alarm = ALARM_COMPLOFF_TIMEOUT_TP5; // T5
				else if ( Error == 656 ) Alarm = ALARM_CONTINUE_TIMEOUT_TP6; // T6
				else if ( Error == 657 ) Alarm = ALARM_AGV_COMM_ERROR;
				else if ( Error == 658 ) Alarm = ALARM_ABNORMAL_FOUPCHANGE;
				else if ( Error == 680 ) Alarm = ALARM_VALIDOFF_ERROR;
				else if ( Error == 681 ) Alarm = ALARM_HOAVBLOFF_ERROR;
				else if ( Error == 682 ) Alarm = ALARM_CSOFF_ERROR;
				else if ( Error == 683 ) Alarm = ALARM_CSOFF_ERROR;
				else if ( Error == 684 ) Alarm = ALARM_TRREQOFF_ERROR;
				else if ( Error == 685 ) Alarm = ALARM_BUSYOFF_ERROR;
				else if ( Error == 686 ) Alarm = ALARM_COMPLOFF_ERROR;
				else if ( Error == 687 ) Alarm = ALARM_CONTOFF_ERROR;
			}
			AlmResult = ALARM_MANAGE_FREE( ALARM_START_POINTER + Alarm );
			WRITE_DIGITAL( PIO_ERR_CLEAR , RUN , &CommStatus );
			if ( AlmResult == ALM_IGNORE ) return AGV_RECOVERY_SCENARIO( LOAD );
			else return SYS_ABORTED; // retry and abort
		}
		else {
			if ( AGV_Sts == READY ) {
				if ( READ_DIGITAL_TRUE( LP_AGV_STS , &AGV_Sts ) == SYS_ABORTED ) return SYS_ABORTED;
				if ( AGV_Sts == READY ) break;
			}
		}

		if ( Event ) {
			if ( READ_DIGITAL_TRUE( LP_AGV_INPUT_TRREQ , &Data ) == SYS_ABORTED ) return SYS_ABORTED;
			if ( Data == ON ) {
				Event = FALSE;
		...
		if ( SIMULATION_AGV_MODE ) {
			if ( READ_DIGITAL( LP_AGV_INPUT_VALID	, &CommStatus ) == ON      ) WRITE_DIGITAL( LP_AGV_OUTPUT_LREQ  , ON  , &CommStatus );
			if ( READ_DIGITAL( LP_AGV_INPUT_TRREQ	, &CommStatus ) == ON      ) WRITE_DIGITAL( LP_AGV_OUTPUT_READY , ON  , &CommStatus );
			if ( READ_DIGITAL( LP_FOUP_STS			, &CommStatus ) == PRESENT ) WRITE_DIGITAL( LP_AGV_OUTPUT_LREQ  , OFF , &CommStatus );
			if ( READ_DIGITAL( LP_AGV_INPUT_COMPLETE, &CommStatus ) == ON      ) WRITE_DIGITAL( LP_AGV_OUTPUT_READY , OFF , &CommStatus );
			if ( READ_DIGITAL( LP_AGV_INPUT_CS		, &CommStatus ) == OFF	   ) {
				WRITE_DIGITAL( LP_AGV_STS , READY , &CommStatus ); break;
			}
		}
	}

	return SYS_SUCCESS;
*/ // jelee Delete End 2005.08.08
}
//------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------
...
//		WRITE_DIGITAL( UNCLAMP_SET , ON  , &CommStatus );
		...
		while ( TRUE ) {
			...
			//if ( READ_DIGITAL( LP_AGV_INPUT_VALID , &CommStatus ) == OFF ) {
			// 2015.11.23 Abnormal Case Alarm : TA2		
			if ( READ_DIGITAL( LP_AGV_INPUT_CS , &CommStatus )    == OFF	|| SIG_VALID == OFF   || 
				 READ_DIGITAL( LP_AGV_INPUT_TRREQ , &CommStatus ) == OFF	|| READ_DIGITAL( LP_AGV_INPUT_COMPLETE , &CommStatus ) == ON ||
				 READ_DIGITAL( LP_AGV_INPUT_BUSY  , &CommStatus ) == ON ){
...
//				LOG(pszFtnName, ELT_File, "[LPM%d] FOUP Sensor Error When UnLoad Sequence------>6", CASSETTE_INDEX);
			...
			if ( READ_DIGITAL( LP_CLAMP_STS , &CommStatus ) == sUNCLAMP ) {
				...
				sprintf( StrBuffer , EVENT_STRING_UNCLAMP_COMPLETE , CASSETTE_INDEX );     //2014.06.25
	...
	while( TRUE ) {
		...
		if ( AGV_Sts == ERROR1 ) { 
			...
			if ( ALARM_MESSAGE_REMAPPING( ALARM_START_POINTER + ALARM_PORT_AGV_ERROR , &Error ) ) {
				...
				else if ( Error == 653 ) Alarm = ALARM_FOUPREMOVE_TIMEOUT_TP3; // T3
				else if ( Error == 654 ) Alarm = ALARM_COMPLETE_TIMEOUT_TP4;   // T4
				else if ( Error == 655 ) Alarm = ALARM_COMPLOFF_TIMEOUT_TP5;   // T5
				else if ( Error == 656 ) Alarm = ALARM_CONTINUE_TIMEOUT_TP6;   // T6
				else if ( Error == 657 ) Alarm = ALARM_AGV_COMM_ERROR;
			...
			AlmResult = ALARM_MANAGE_FREE( ALARM_START_POINTER + Alarm );
			WRITE_DIGITAL( PIO_ERR_CLEAR , RUN , &CommStatus );
			if ( AlmResult == ALM_IGNORE ) return AGV_RECOVERY_SCENARIO( UNLOAD );
			else return SYS_ABORTED; // retry and abort
		}
		else {
			if ( AGV_Sts == READY ) {
		...
		if ( SIMULATION_AGV_MODE ) {
			if ( READ_DIGITAL( LP_AGV_INPUT_VALID	, &CommStatus ) == ON      ) WRITE_DIGITAL( LP_AGV_OUTPUT_UREQ  , ON  , &CommStatus );
			if ( READ_DIGITAL( LP_AGV_INPUT_TRREQ	, &CommStatus ) == ON      ) WRITE_DIGITAL( LP_AGV_OUTPUT_READY , ON  , &CommStatus );
			if ( READ_DIGITAL( LP_FOUP_STS			, &CommStatus ) == ABSENT  ) WRITE_DIGITAL( LP_AGV_OUTPUT_UREQ  , OFF , &CommStatus );
			if ( READ_DIGITAL( LP_AGV_INPUT_COMPLETE, &CommStatus ) == ON      ) WRITE_DIGITAL( LP_AGV_OUTPUT_READY , OFF , &CommStatus );
			if ( READ_DIGITAL( LP_AGV_INPUT_CS		, &CommStatus ) == OFF	   ) {
				WRITE_DIGITAL( LP_AGV_STS , READY , &CommStatus ); break;
			}
...
//...Start 2015.11.23 Add 15 Comm Spec (OHT Unload/Load Interlock)
...
//... End 2015.11.23 Add 15 Comm Spec (OHT Unload/Load Interlock)
...
//			printf( "=====> CM%d Load Handling : 3-3\n" , CASSETTE_INDEX);//jelee
			...
			if ( Data == ON || Data2 == BUSY ) {
				...
				if ( AGV_LOAD_CONTROL() != SYS_SUCCESS ) {
					...
					AGV_INIT_SET_IO( -2 );		//... 2015.11.23 Add 15 Comm Spec (OHT Unload/Load Interlock)
					WRITE_DIGITAL( LP_AMHS_MOVING , OFF , &CommStatus ); // idle
...
//		printf( "=====> CM%d Load Handling : 8\n" , CASSETTE_INDEX);//jelee
		//... 2016.03.14
		if ( READ_DIGITAL_TRUE( LP_FOUP_STS		, &placeSensor  ) == SYS_ABORTED ) return SYS_ABORTED;
		if ( READ_DIGITAL_TRUE( PRESENT_STS		, &presentSensor  ) == SYS_ABORTED ) return SYS_ABORTED;

		if( presentSensor == NOTEXIST || presentSensor == NOTEXIST ) 
		{
			return SYS_ABORTED;
		}
...
/* // jelee Delete Start.
	...
	if      ( STRCMP_L( RunStr , "LOAD"   ) ) {
		WRITE_DIGITAL( LP_LOADING_SIGNAL , 2 , &CommStatus );
		Res = Load_Control();
		WRITE_DIGITAL( LP_LOAD_LAMP , OFF , &CommStatus );
		WRITE_DIGITAL_DOUBLE( LP_UNLOADING_SIGNAL , LP_LOADING_SIGNAL , NONE3 , &CommStatus );
		Current = NONE;
		if ( SIMULATION_AGV_MODE ) AGV_INIT_SET_IO_FOR_SIM();
		return Res;
	}
	else if ( STRCMP_L( RunStr , "UNLOAD" ) ) {
		WRITE_DIGITAL( LP_UNLOADING_SIGNAL , 2 , &CommStatus );
		Res = Unload_Control();
		WRITE_DIGITAL( LP_UNLOAD_LAMP , OFF , &CommStatus );
		WRITE_DIGITAL_DOUBLE( LP_UNLOADING_SIGNAL , LP_LOADING_SIGNAL , NONE3 , &CommStatus );
		Current = NONE;
...
/*	char Str1[31] , Str2[31];
	...
	STR_SEPERATE( PROGRAM_ARGUMENT_READ() , Str1 , Str2 , 30 );
	REPLACE_CHAR_SET( 0 , Str1 );
	REPLACE_CHAR_SET( 1 , Str1+2 );
	CASSETTE_INDEX = atoi( Str1+2 );
	STR_SEPERATE( Str2 , Str1 , Str2 , 30 );
	ALARM_START_POINTER = atoi( Str1 );
*/
}
//---------------------------------------------------------------------------------------
...
BOOL Program_Init_Code() {
	...
	if ( READ_DIGITAL( IO_Driver_Loading , &CommStatus ) == 0 )	{
		...
		/*
		DIGITAL_DRIVER_NAME( LP_FOUP_STS , BufferI , BufferO );
		if ( STRCMP_L( BufferI , "-virtual-" ) || STRCMP_L( BufferI , "" ) ) SIMULATION_CASS_MODE = TRUE;
		DIGITAL_DRIVER_NAME( LP_UNLOAD_BUTTON , BufferI , BufferO );
		if ( STRCMP_L( BufferI , "-virtual-" ) || STRCMP_L( BufferI , "" ) ) SIMULATION_USWITCH_MODE = TRUE;
		DIGITAL_DRIVER_NAME( LP_LOAD_BUTTON , BufferI , BufferO );
		if ( STRCMP_L( BufferI , "-virtual-" ) || STRCMP_L( BufferI , "" ) ) SIMULATION_LSWITCH_MODE = TRUE;
		while ( TRUE ) {
			DIGITAL_DRIVER_NAME( LP_AGV_INPUT_CS , BufferI , BufferO );
			if ( !STRCMP_L( BufferI , "-virtual-" ) && !STRCMP_L( BufferI , "" ) ) break;
			DIGITAL_DRIVER_NAME( LP_AGV_INPUT_CS1 , BufferI , BufferO );
			if ( !STRCMP_L( BufferI , "-virtual-" ) && !STRCMP_L( BufferI , "" ) ) break;
			DIGITAL_DRIVER_NAME( LP_AGV_INPUT_VALID , BufferI , BufferO );
			if ( !STRCMP_L( BufferI , "-virtual-" ) && !STRCMP_L( BufferI , "" ) ) break;
			DIGITAL_DRIVER_NAME( LP_AGV_INPUT_TRREQ , BufferI , BufferO );
			if ( !STRCMP_L( BufferI , "-virtual-" ) && !STRCMP_L( BufferI , "" ) ) break;
			DIGITAL_DRIVER_NAME( LP_AGV_INPUT_BUSY , BufferI , BufferO );
			if ( !STRCMP_L( BufferI , "-virtual-" ) && !STRCMP_L( BufferI , "" ) ) break;
			DIGITAL_DRIVER_NAME( LP_AGV_INPUT_COMPLETE , BufferI , BufferO );
			if ( !STRCMP_L( BufferI , "-virtual-" ) && !STRCMP_L( BufferI , "" ) ) break;
			DIGITAL_DRIVER_NAME( LP_AGV_INPUT_CONT , BufferI , BufferO );
			if ( !STRCMP_L( BufferI , "-virtual-" ) && !STRCMP_L( BufferI , "" ) ) break;
			SIMULATION_AGV_MODE = TRUE;
			break;
		}
	...
	/*
	if ( !EXCHANGE_SIM_SHOW ) {
		if ( SIMULATION_AGV_MODE || SIMULATION_CASS_MODE || SIMULATION_LSWITCH_MODE || SIMULATION_USWITCH_MODE ) {
			EXCHANGE_SIM_SHOW = TRUE;
		}
	}
	*/
