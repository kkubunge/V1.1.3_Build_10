...
enum
{
	...
	ROBOT_SYNCH_RESET_REVERSE,
	ROBOT_SYNCH_RESET_ONLY,
	ROBOT_SYNCH_SUCCESS
};
//------------------------------------------------------------------------------------------
enum
...
IO_Name_String_Map IO_STR_Map_Table[] = {
	...
	{"ULVAC_Run_Status", _K_D_IO, ULVAC_Run_Status, 0},
...
BOOL CheckAbortControl()
{
	...
	{
		if (g_DynIO.dREAD_DIGITAL(CTC_AbortControl, &bIOStatus) != ABC_IDLE)
			bRet = TRUE;
	}
	return bRet;
}
			  ...
			  char *szData, BOOL bDualArm)
{
	...
	do
	{
		if (STRCMP_L(szRunStr, "PICK"))
			strcpy(szEventID, "Get");
		else if (STRCMP_L(szRunStr, "PLACE"))
			strcpy(szEventID, "Put");
		else if (STRCMP_L(szRunStr, "ROTATE"))
			strcpy(szEventID, "Rotate");
		else
			break;
		...
		if (!bDualArm)
			_MLOG("XFR %s %s %s %s %d %c %s", szModule, szEventID, szStatus, szDeviceID, nSlot, 'A' + nArmIdx, szTemp);
		else
			_MLOG("XFR %s %s %s %s [2,%d,%d] %c %s", szModule, szEventID, szStatus, szDeviceID, nSlot, nSlot + 1, 'A' + nArmIdx, szTemp); // 2013.07.29
	} while (0);
}
...
Module_Status READ_AXIS_LOAD()
{
	...
	int ReadCnt = 0, AlmResult, i, AlmRange, ReadIO;
	char ReadStr[51], TempStr1[51], TempStr2[52], AlmMsg[256], AlmAxis[20];
	...
	//	for ( i=0 ; i<7 ; i++ )
	//	{
	//		DIGITAL_NAME ( ULVAC_LOAD_Ti + i , IOName ) ;
	//		printf ( "%s:%d\n" , IOName , READ_DIGITAL ( ULVAC_LOAD_Ti + i , &CS ) ) ;
	//	}
	...
	while (TRUE)
	{
		...
		{ //Got Fault or error occur
			...
			if (1 == errlevel)
			{
				...
				{
					if (ALARM_NUMBER[i] == error)
					{
						error_code = i + 101;
						break;
					}
					else
						error_code = ALARM_OTHER;
				}
			}
			else if (2 == errlevel)
			{
		...
		{ //Got Invalid Data or Disconnected
...
//------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------
Module_Status Robot_AlarmClear()
{
	//This function created for alarm clear before vac grip check
	//but now actually no need
	Module_Status msRet = SYS_SUCCESS;
	int CommStatus;
	//Reset
	WRITE_DIGITAL(MTR5_T_Error_Clear, 0, &CommStatus);
	if (!CommStatus)
	{
		_LOG("----->Auto Recovery IO Operation Fail in Robot_Auto_Recovery[Reset]");
		return SYS_ABORTED;
	}
	...
	int rejectalarm)
{
	...
	if (Last_Retract >= 0)
		WRITE_DIGITAL(ROBOT_RET, 1, &CommStatus);
	...
	do
	{
		...
		{
			if (Synch_Start >= 0)
				WRITE_DIGITAL(ROBOT_SYNCH, Synch_Start, &CommStatus);
		}
		/////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if (Arm == ARM_A)
			WRITE_DIGITAL(A_IOP, Station, &CommStatus);
		else
			WRITE_DIGITAL(B_IOP, Station, &CommStatus);
		...
		do
		{
			...
			if (Check == SYS_ABORTED)
			{
				msRet = SYS_ABORTED;
				break;
			}
			else if (Check == SYS_RUNNING)
				break;

			if (Data == 0)
			{
				if (Last_Retract >= 0)
					WRITE_DIGITAL(ROBOT_RET, Last_Retract, &CommStatus);
				if (Synch_End >= 0)
					WRITE_DIGITAL(ROBOT_SYNCH, Synch_End, &CommStatus);
				{
					msRet = SYS_SUCCESS;
					break;
				}
		...
		else
		{
			if (DB)
			{
				if (RobotMovement(Station, Arm,
								  ...
								  ALARM_PICK_WAFER,
								  ROBOT_SYNCH_RETRACT_START, ROBOT_SYNCH_RETRACT_START, 0,
								  ROBOT_STATUS_RETRACT, Station, -1, ROBOT_STATUS_UP, -1) == SYS_ABORTED)
					break;
			}
			else
			{
	...
	do
	{
		...
		if (DB)
		{
			if (RobotMovement(Station, Arm,
							  ...
							  ALARM_PLACE_WAFER,
							  ROBOT_SYNCH_RETRACT_START, ROBOT_SYNCH_RETRACT_START, 0,
							  ROBOT_STATUS_RETRACT, Station, -1, ROBOT_STATUS_DOWN, -1) == SYS_ABORTED)
				break;
		}
		else
		{
...
BOOL Find_Chamber_Finger_From_String(char *String, int *EStation, int *Station, int *Arm, char *SlotString, int *Slot)
{
	...
	if (STRCMP_L(String + offset, "CM1"))
	{
		*EStation = CM1;
	}
	else if (STRCMP_L(String + offset, "CM2"))
	{
		*EStation = CM2;
	}
	else if (STRCMP_L(String + offset, "CM3"))
	{
		*EStation = CM3;
	}
	else if (STRCMP_L(String + offset, "AL"))
	{
		*EStation = AL;
	}
	else if (STRCMP_L(String + offset, "BM1"))
	{
		*EStation = BM1;
	}
	else if (STRCMP_L(String + offset, "BM2"))
	{
		*EStation = BM2;
	}
	else
...
void PostPickEvent(int Station, int Arm, BOOL DB)
{
	...
	if (Arm == ARM_A && DB == FALSE)
	{
		...
		if ((Station == 6 || Station == 7) && (nWaferResult1 != 0 || nWaferResult2 != 0))
		{
...
Module_Status Simulation_Main()
{
	...
	// 2013.11.15
	if (STRCMP_L(ElseStr, "TRANSFER2"))
		TR = TRUE;
	else if (STRCMP_L(ElseStr, "DB"))
		DB = TRUE;
	...
	do
	{
		...
		if (sp > 0)
			WRITE_DIGITAL(MTR5_T_Slot_Pos, sp, &CommStatus);
		...
		{
			PostPlaceEvent(Station, Arm, DB, TRUE);
		}
		// 2005.12.09
	...
	if (SYS_SUCCESS == msRet)
		WRITE_DIGITAL(ROBOT_SYNCH, ROBOT_SYNCH_SUCCESS, &Station);
...
Module_Status Real_Main()
{
	...
	if (READ_DIGITAL(CONTROL, &Station) == 1)
		return SYS_SUCCESS;
	...
	do
	{
		...
		*/
		...
		else if (STRCMP_L(RunStr, "PLACE"))
		{
			...
			msRet = RotPicRobot(Station, Arm);
		else if (STRCMP_L(RunStr, "ROTPLC"))
			msRet = RotPlcRobot(Station, Arm);
		else
		{
...
extern "C"
{
	...
	void Program_Enter_Code()
	{
		...
		//---------------------------------------------------
		if (strlen(PROGRAM_ARGUMENT_READ()) <= 0)
			return;
