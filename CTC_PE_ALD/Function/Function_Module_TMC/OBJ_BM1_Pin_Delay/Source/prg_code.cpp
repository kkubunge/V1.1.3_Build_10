...
IO_Name_String_Map IO_STR_Map_Table[] = {
	...
	{ "FC_BM1_PIN_CTRL"		  , _K_D_IO	,	CONTROL				    ,	0	} ,

	{ "PRMD_BM1_PIN_TMO"	  , _K_D_IO	,	CASSETTE_TIMEOUT	    ,	0	} ,
	// PRMD_BM1_PIN_TD, Pin Down Delay Time
	{ "PRMA_BM1_PIN_TD" 	  , _K_A_IO	,	LL_PINDOWN_DELAYTIME 	,	0	} ,

	{ "BM1.PinUpDownLXO"	  , _K_D_IO	,	LL_PinUpDownLXO			,	0	} ,
	{ "BM1.PinUpDownRXO"	  , _K_D_IO	,	LL_PinUpDownRXO			,	0	} ,
	{ "BM1.PinUpDownLXI"	  , _K_D_IO	,	LL_PinUpDownLXI			,	0	} ,
	{ "BM1.PinUpDownRXI"	  , _K_D_IO	,	LL_PinUpDownRXI			,	0	} ,
...
//	printf("[Debug] Entered %s\n", APP_NAME);
	...
	if ( SIMULATION_MODE )
	{
		...
		if(! g_DynIO.dRUN_FUNCTION(TMSIM_BM1, szCmd)) return SYS_ABORTED;
		return SYS_SUCCESS;
	}
	...
	if      ( STRCMP_L( RunStr , "A_UP"     ) ) {
		...
		if ((READ_DIGITAL( CTC_BM1_Wafer_Status  , &CommStatus) >= 1 ) && 
			(READ_DIGITAL( CTC_BM1_Wafer_Status  , &CommStatus) <= 25 ) ||
			(READ_DIGITAL( CTC_BM1_Wafer_Status2 , &CommStatus) >= 1 ) && 
			(READ_DIGITAL( CTC_BM1_Wafer_Status2 , &CommStatus) <= 25 )) {
			return (PIN_LL(1));
		}
		else return SYS_SUCCESS;
	}
