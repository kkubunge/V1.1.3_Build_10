...
// Added by jhlee. 2010.03.22
Module_Status FOUP_Door_Control(int mode) // 1:close, 2:open
{
	int CommStatus;	

	if(READ_DIGITAL(LPM_DoorUpDownXI, &CommStatus) == mode )
		return SYS_SUCCESS;

	WRITE_DIGITAL(LPM_FOUPDoor, mode, &CommStatus);
	if(mode == 1)
	{
		if ( Run_Part( -1 , -1 , -1 , -1 , -1 ,
					-1 , -1 , -1 , -1 , FALSE , 
			...
			if ( STRCMP_L( RunStr , "MAPPING"   ) ) 
			{
				...
				if ( STRCMP( SlotStr , "IN"  ) ) {
					if ( strlen( ElseStr ) > 0 ) {
						...
						sprintf(szCmd,"CM %s IN", RunStr);


						if (StationStr[2] == "1"){
							RUN_FUNCTION(TMSIM_CM1, szCmd);				
						}else if(StationStr[2] == "2"){
							RUN_FUNCTION(TMSIM_CM2, szCmd);	
						}else if(StationStr[2] == "3"){
							RUN_FUNCTION(TMSIM_CM2, szCmd);	
						}
