...
//////////////////////////////////////////////////////////////////////////
Module_Status Alarm_Clear(int Station) 
{
	int		i;	
	...
	for(i = 0; i < nTotalAlmCount; i++)
	{
		...
		printf("[ALARM] Read AlarmID[%d] Module[%d]\n", nAlarmID[i], nModuleNo[i]);
	}

	//... Clear Alarm by Module
	for(i = 0; i < nTotalAlmCount; i++) 
	{
		bClearFlag = FALSE;		//.. Initial Clear Flag

		if     (Station == PM12 && (nModuleNo[i] == 1 || nModuleNo[i] == 2))		bClearFlag = TRUE;
		else if(Station == PM34 && (nModuleNo[i] == 3 || nModuleNo[i] == 4))		bClearFlag = TRUE;
		else if(Station == PM56 && (nModuleNo[i] == 5 || nModuleNo[i] == 6))		bClearFlag = TRUE;
		else if(Station == TMC  &&  nModuleNo[i] == 0)								bClearFlag = TRUE;

		//... 2015.02.07 Add Process Pause Alarm Clear Skip
		if(nAlarmID[i] == ALARM_PM_PROC_PAUSE && 
			(Station == PM12 || Station == PM34 || Station == PM56))
		{
			bClearFlag = FALSE;
		}
		...
		if(bClearFlag)
		{
			...
			printf("[ALARM] Module[%d] AlarmID[%d] Cleared\n", nModuleNo[i], nAlarmID[i]);
		}
	}
...
Module_Status Program_Main() {
	...
	{
		Station = READ_DIGITAL(CTC_ALARM_STATION, &CommStatus);

		switch(Station) 
		{
			case TMC	:	Result = Alarm_Clear(TMC);		break;
			case PM12	:	Result = Alarm_Clear(PM12);		break;
			case PM34	:	Result = Alarm_Clear(PM34);		break;
			case PM56	:	Result = Alarm_Clear(PM56);		break;
			default		:	Result = Alarm_Clear_All();		break;
		}
	}
