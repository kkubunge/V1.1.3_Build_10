...
#define		DISPLAY_START_X_FOR_ANALOG_NAME		640
//------------------------------------------------------------------------------------------
#define		DISPLAY_START_X_FOR_ANALOG_DATA		770
//------------------------------------------------------------------------------------------
...
#define		DISPLAY_START_X_FOR_ANALOG_HELP		86
//------------------------------------------------------------------------------------------
#define		DISPLAY_SIZE_X_FOR_ANALOG_NO		3
#define		DISPLAY_SIZE_X_FOR_ANALOG_NAME		27
...
//------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------
BOOL READ_AIO_INFORMATION( char *Name , char *Show , int Point , char *Help , BOOL Ana , int Tgl ) {
	int Type , min , max;
	int Out_Dev;
	char EnumList[1024];
	...
	for ( Line = 1 ; FileEnd ; Line++ ) {
		...
		if ( ReadCnt > 0 ) {
			...
			if ( STRCMP_L( "$_PLOT_BIG" , chr_return1 ) ) {
				...
				//
				if ( !Get_Data_From_String_for_INT( Buffer , 6 , chr_return1 , 1 , 12 , &(A_IO_PLT_DEV(Point)) ) ) {
		...
		if ( ReadCnt > 0 ) {
			...
			if      ( STRCMP_L( "$HEADER_MESSAGE" , chr_return1 ) ) {
					...
					if ( ReadCnt > 2 ) {
						...
						if ( ReadCnt > 3 ) {
							if ( !Get_Data_From_String( Buffer , chr_return2 , 3 ) ) {
								_IO_CIM_PRINTF( "Unknown Error 3 - [%s]-[%d]\n" , FileName , Line );
								_lclose( hFile );
								return FALSE;
							}
				...
				if ( !Get_Data_From_String_for_INT( Buffer , 1 , chr_return1 , 0 , 999 , &DATALOG_TIME ) ) {
					_IO_CIM_PRINTF( "Datalog time Data Error(1-999)- [%s]-[%d]\n" , FileName , Line );
					_lclose( hFile );
					return FALSE;
				}
			}
				...
				if ( !Get_Data_From_String_for_INT( Buffer , 1 , chr_return1 , 1 , 999 , &DATALOG_AUTO_DELETE_TIME ) ) {
					_IO_CIM_PRINTF( "Datalog auto delete time Data Error(1-999)- [%s]-[%d]\n" , FileName , Line );
					_lclose( hFile );
					return FALSE;
				}
			}
				...
				if ( !GuiDraw ) {
					...
					if ( !KGUI_PALETTE_Read_PalletteTable_File( NULL , chr_return2 ) ) {
						_IO_CIM_PRINTF( "Open File Error - [%s]-[%d]\n" , FileName , Line );
						_lclose( hFile );
						return FALSE;
					}
				}
				...
				if ( !READ_AIO_INFORMATION( chr_return1 , chr_return3 , Point , chr_return2 , Tp , Tgl ) ) return FALSE;
			}
		}
	}
	_lclose( hFile );
