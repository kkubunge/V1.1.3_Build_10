...
int CTextParser::FindWord(const char *szWord)
{
	int nIdx = 0;

	MoveFirst();
	do {
		if(! GetWord()) { nIdx = -1; break; }
		if(BeEqual(szWord)) break;
		nIdx++;
	} while(1);
	return nIdx;
}

int CTextParser::FindString(const char *szWord)
{
	int i, nIdx = 0;

	for(i=0; i<(int)strlen(m_pText); i++)
	{
		if(BeEqual(m_pText+i, szWord, strlen(szWord))) break;
	}
	if(i >= (int)strlen(m_pText)) nIdx = -1;
	else nIdx = i;
	return nIdx;
}
