...
// #include "MsgBoxDll.h"
#ifdef _MSGBOXDLL_H_
#define _ERRMSG(_SZ_MSG)	_gOutMsgBox("CMonThread", _SZ_MSG, FALSE)
#endif
...
void CMonThread::DestroyAllThread(int nWaitTime)
{
	int i;
	for(i=0; i<MAX_THREAD; i++)
			if(m_pnThreadSts[i] == THRD_STS_RUN) m_pnThreadSts[i] = THRD_STS_STOP;		// 2008.12.03
...
void CMonThread::DoMonitor(int nThreadNo)
{
	switch(nThreadNo)
	{
	case THRD0_Monitor_Func: DoMonitor(); break;	/* THRD0_Monitor_Func_0 = nThreadNo = 0: Default Monitor Function */
	case THRD1_Monitor_Func: DoMonitor_1(nThreadNo); break;
	case THRD2_Monitor_Func: /* DoMonitor_2(nThreadNo); */ break;
	case THRD3_Monitor_Func: /* DoMonitor_3(nThreadNo); */ break;
	}
}

void CMonThread::DoMonitor_1(int nThreadNo)			// Sample Code
{
	while(WAIT_SEC(1))
	{
		// Monitor Code
...
{
	int i;
	for(i=0; i<nSec*10 && m_pnThreadSts[nThreadNo] == THRD_STS_RUN; i++) Sleep(100);	// 2008.12.03
...
bool CMonThread::IsThreadRunning(int nThreadNo)
{
	if(m_pnThreadSts[nThreadNo] == THRD_STS_RUN) return true;
	return false;
}