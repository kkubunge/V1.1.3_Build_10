...
public:
	...
	char *dREAD_STRING_FROM_NAME(char *szIOName, char *pValue, BOOL *pResult);	// 2013.07.02
	...
	// 2010.04.16
	void dWRITE_IO_FROM_NAME(char *szIOName, int nIOType, double dbValue, BOOL *pResult);

	//-----------------------------------------------------------------
	// 2010.04.07
	void dWRITE_IO(int nIoIndex, int nIOType, double dbValue, BOOL *pResult);
	double dREAD_IO(int nIoIndex, int nIOType, BOOL *pResult);
...
2010.04.07	READ_IO(...), WRITE_IO(...) Func 추가
2010.04.16	dWRITE_IO_FROM_NAME Func 추가
			Value 인자를 한가지만 가지는 Function임
			InitIOChannel() Func 코드 오류 수정
2013.07.02	dREAD_STRING_FROM_NAME(...) Func 추가
--------------------------------------------------------------------*/
