...
void CDynamicIO::ClearAndInitIOList(IO_LIST pIOList[], int nCount)						// 2008.12.17
{
	int i;
	if(nCount < 1) return;
	for(i=0; i<nCount; i++)
	{
		strcpy(pIOList[i].szIOName, "NotDefined");
		pIOList[i].nIOType = -1;
		pIOList[i].nIOPoint = -1;
	}
	strcpy(pIOList[nCount-1].szIOName, "");
	InitIOList(pIOList);
}
...
int CDynamicIO::RegistIOChannel(const char *szIOName, int nIOType)	// 2006.03.30
{
	...
	for(i=0; m_pIOList[i].szIOName[0] != 0; i++)
	{
		...
		{
			nIoIdx = i;
			strcpy(m_pIOList[nIoIdx].szIOName, szIOName);
			m_pIOList[nIoIdx].nIOType = nIOType;
			break;
		}
	}
	...
	if(nIoIdx < 0)
	{
		printf("--------> There Is No I/O Area 'NotDefined' in RegistIOChannel\n");
	}
	...
	if(nIoIdx >= 0)
	{
		...
		{
			printf("--------> InitIOChannel[%s,%d] Failed in RegistIOChannel\n", szIOName, nIOType);
			strcpy(m_pIOList[nIoIdx].szIOName, "NotDefined");
			nIoIdx = -1;
		}
	}
	return nIoIdx;
...
//-----------------------------------------------------------------
// 2013.07.02
char* CDynamicIO::dREAD_STRING_FROM_NAME(char *szIOName, char *pValue, BOOL *pResult)
{
	int nIOPoint;
	nIOPoint = _FIND_FROM_STRING(_K_S_IO, szIOName);
	if(nIOPoint != -1)
	{
		pValue = dREAD_STRING(nIOPoint, pValue, pResult);
	}
	return pValue;
}
...
void CDynamicIO::dWRITE_IO_FROM_NAME(char *szIOName, int nIOType, double dbValue, BOOL *pResult)	// 2010.04.16
{
	switch(nIOType)
	{
	case _K_D_IO: dWRITE_DIGITAL_FROM_NAME(szIOName, (int) dbValue, pResult); break;
	case _K_A_IO: dWRITE_ANALOG_FROM_NAME(szIOName, dbValue, pResult); break;
	default : *pResult = FALSE; break;
	}
}
...
void CDynamicIO::dWRITE_IO(int nIoIndex, int nIOType, double dbValue, BOOL *pResult)
{
	switch(nIOType)
	{
	case _K_D_IO: dWRITE_DIGITAL(nIoIndex, (int)dbValue, pResult); break;
	case _K_A_IO: dWRITE_ANALOG(nIoIndex, dbValue, pResult); break;
	default : *pResult = FALSE; break;
	}
}

double CDynamicIO::dREAD_IO(int nIoIndex, int nIOType, BOOL *pResult)
{
	double dbRet = 0;
	switch(nIOType)
	{
	case _K_D_IO: dbRet = dREAD_DIGITAL(nIoIndex, pResult); break;
	case _K_A_IO: dbRet = READ_ANALOG(nIoIndex, pResult); break;
	default : *pResult = FALSE; break;
	}
	return dbRet;
}
