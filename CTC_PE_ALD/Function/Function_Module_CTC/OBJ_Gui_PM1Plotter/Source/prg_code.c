...
IO_Name_String_Map IO_STR_Map_Table[] = {
	...
	{ "CTC.PM1_Wafer_Status"	, _K_D_IO	,		Wafer_Status		,	0	} ,
	{ "CTC.PM1_Wafer_Source"	, _K_D_IO	,		Wafer_Source		,	0	} ,
	{ "CTC.PM1_RECIPE_FILE"		, _K_S_IO	,		PROCESS_RECIPE		,	0	} ,
	{ "PM1.Temp_Log_FileName"	, _K_S_IO	,		TEMP_LOG_FILENAME	,	0	} ,
	""
};
//------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------
...
void Plotter_Pre_Polling() {
	...
	int DelDay , n , DelResult;
	int DestYear , DestMonth , DestDay , DestWeek;
	int Month_Su[]={ 31 , 31 , 28 , 31 , 30 , 31 , 30 , 31 , 31 , 30 , 31 , 30 , 31 };	
		...
		if ( DISPLAY_CONTROL >= 0 ) {
			...
			if ( !MAIN_RUNTIME ) {
				if ( ( d >= 1 ) && ( d <= 6 ) ) { // 2002.04.02
					...
					InvalidateRect( PLOT_MAIN_WINDOW , NULL , TRUE );
				}
			}
			else {
				if ( ( d < 1 ) || ( d > 6 ) ) { // 2002.04.02
...
/*		if ( t >= 7200 ) { // d / 2 = sec
			...
			if ( DATALOG_CONTROL != -1 ) {
				i = File_Delete_for_Time_Reached( DATALOG_DIRECTORY_NAME() , "*.log" , DATALOG_AUTO_DELETE_TIME );
				if ( i != 0 ) {
...
*/
		...
		if( t >= 7200 ) {
			...
			DelDay = DATALOG_AUTO_DELETE_TIME;
			if( DelDay >= 365 ) DelDay = 365;

			DestYear = SysTime.wYear;
			DestMonth = SysTime.wMonth;
			DestDay = SysTime.wDay;
			DestWeek = SysTime.wDayOfWeek;

			for( n = DelDay ; n > 0 ; n-- ) {
				DestDay--; DestWeek--;
				if( DestYear % 4 == 0 && DestYear % 100 != 0 || DestYear % 400 == 0 ) Month_Su[2] = 29; // À±´Þ 
				if( DestDay   <  1 ) { DestMonth--; DestDay = Month_Su[DestMonth]; }
				if( DestMonth == 0 ) { DestYear--; DestMonth = 12; DestDay = 31; }		
				if( DestWeek  <  0 ) DestWeek = 6;		
			}

			sprintf( Buffer , "%s\\%d_%02d\\%02d_%s" , DATALOG_DIRECTORY_NAME() , DestYear , DestMonth , DestDay , Week[DestWeek] );
			DelResult = Directory_Delete_at_Sub( Buffer );

			if( DelResult == 1 ) printf("<PLOTTER> Plotting Logging Data Deleted [Datalog/PM2/%d_%02d/%02d_%s]\n", DestYear , DestMonth , DestDay , Week[DestWeek] );

			if( DestMonth == 1 ) {
				DestYear = DestYear - 1;
				DestMonth = 12;
			}
			else DestMonth = DestMonth - 1;

			sprintf( Buffer , "%s\\%d_%02d" , DATALOG_DIRECTORY_NAME() , DestYear , DestMonth );
			DelResult = Directory_Delete_at_Sub( Buffer );

			if( DelResult == 1 ) printf("<PLOTTER> Plotting Logging Data Deleted [Datalog/PM2/%d_%02d]\n", DestYear , DestMonth );
		}
		t++;
	}
	_endthread();
...
void Plotter_Data_Polling() {
	...
	char Buffer1[128] , Buffer2[128] , Type;
	int i , j , k , Point , IOP;
			...
			if ( _dGROUP_IO_READ_from_MEM( Set_IO_Status , Return_IO_Status , 5120 ) ) {
				...
				while ( _dGROUP_IO_TRANSLATE_from_BUFFER( Return_IO_Status , i , &Type , &Point , Buffer1 , Buffer2 , &k ) ) {
					...
					case 'S' :
						...
						break;
					default :
						for ( j = 0 ; j < ANALOG_CNT ; j++ ) {
							if ( A_IO_POINT( j ) == Point ) break;
						}
						if ( j == ANALOG_CNT ) IOP = -1;
						else                   IOP = j;
						break;
					}
					if ( IOP != -1 ) {
...
//	REGIST_FILE_INFO( "Gui Plotter 3.5(2002.08.08)" ); // Datalog IO Expand
	...
	//----------------------------------
	STR_SEPERATE_CHAR( PROGRAM_ARGUMENT_READ() , '|' , Else1Str , Else2Str , 255 );
	//>>-------------------------------------------------------------------------
	...
	GoModalDialogBoxParam( GETHINST(hWnd) , MAKEINTRESOURCE( IDD_CONTROL_PAD ) , hWnd , Console_Gui_For_PlotMain_Proc , (LPARAM) NULL );
