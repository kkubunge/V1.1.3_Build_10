...
BOOL CModuleConditionControl_Client::CheckChamberEnable(BOOL bAutoControl)
{
	...
	{
		// Leak Check Running(2)
		if(2 == READ_DIGITAL(LeakCheckSts, &g_nCommStatus))				return FALSE;

		//... 2015.04.18 Add Idle Timer Stop for Auto Cycle Clean
		// Case. Reserved Auto Cycle Clean
		if(3 == READ_DIGITAL(PRMD_PM_AutoCycRun, &g_nCommStatus))		return FALSE;
	}
...
//			if (FALSE == CheckDRYCleaningCondition(bAutoControl))	break;
		...
		{
			//WRITE_DIGITAL(ModeChangeIgnore, __ON, &g_nCommStatus); //... 2018.11.09 Not Use
