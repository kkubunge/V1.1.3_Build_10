...
static void _Rate_LOG(const char * pszAppName, ELogType eLogOption, const char * pFormat, ...)
{
	// 0. declaration
	va_list args;		
	char szFileName[256];
	...
	if( ELT_File == eLogOption || ELT_FileConsole == eLogOption)
	{
		...
		/*
		sprintf( szFileName, "D:\\Bluetain-CTC\\_log\\OperRateLog_%04d%02d%02d.log",														
															strtSysTime.wYear,
															strtSysTime.wMonth, 
															strtSysTime.wDay);
		*/													
		
		sprintf( szFileName, "D:\\Bluetain-CTC\\_log\\OperRateLog_%04d%02d.log",														
															strtSysTime.wYear,
															strtSysTime.wMonth);
