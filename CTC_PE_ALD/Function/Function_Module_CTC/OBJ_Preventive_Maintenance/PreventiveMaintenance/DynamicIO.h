...
public:
	...
	// Member Function
public:
	BOOL InitIOList(IO_LIST pIOList[], bool bIOValidErrCheck = true, int nIOValidCheckIdx = 0, int nModuleNumber = 0);
...
private:
	void ChangeChar(char* szString, char cCur, char cNew);		// 2006.02.22
...
2005.12.20	(1) <cskim> 동적으로 Function을 사용할 수 있는 기능 추가
			- dRUN_FUNCTION(...)
			- dRUN_FUNCTION_FREE(...)
			- dREAD_FUNCTION(...)
2005.12.20	(2) <cskim> dRUN_FUNCTION(...) Function의 return 값을 갖도록 수정.
			dRUN_WAIT_FUNCTION(...) 추가
2006.02.22	<cskim> InitIOList(...) Function에서 I/O Name의 일부분을 Module Number
			로 치환할 수 있는 기능 추가
			I/O Name Define 시에 '#' 문자를 쓰면 Module Number로 치환된다.
--------------------------------------------------------------------*/
