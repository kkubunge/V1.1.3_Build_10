...
static void LOG(const char * pszAppName, ELogType eLogOption, const char * pFormat, int CASSETTE_INDEX, ...)
{
	// 0. declaration
	va_list args;		
	char szFileName[256];
	...
	sprintf( szTimeStamp, "%dh:%02dm:%02dsec:%03dms",	strtSysTime.wHour,
														strtSysTime.wMinute,
														strtSysTime.wSecond,
	...
	if( ELT_File == eLogOption || ELT_FileConsole == eLogOption)
	{
		...
		if( CASSETTE_INDEX == 1 )
		{
//			sprintf( szFileName, "D:\\Bluetain_CTC\\_Log\\PIOViewer\\LPMA\\~$%s_%04dy%02dm%02dd.log", 															
			sprintf( szFileName, "_Log\\PIOViewer\\LPMA\\~$%s_%04dy%02dm%02dd.log", 															
				szFileAppName,
				strtSysTime.wYear,
				strtSysTime.wMonth, 
				strtSysTime.wDay);
		}
		else if( CASSETTE_INDEX == 2 )
		{
//			sprintf( szFileName, "D:\\Bluetain_CTC\\_Log\\PIOViewer\\LPMB\\~$%s_%04dy%02dm%02dd.log", 															
			sprintf( szFileName, "_Log\\PIOViewer\\LPMB\\~$%s_%04dy%02dm%02dd.log", 															
				szFileAppName,
				strtSysTime.wYear,
				strtSysTime.wMonth, 
				strtSysTime.wDay);
		}
		else if( CASSETTE_INDEX == 3 )
		{
//			sprintf( szFileName, "D:\\Bluetain_CTC\\_Log\\PIOViewer\\LPMC\\~$%s_%04dy%02dm%02dd.log", 															
			sprintf( szFileName, "_Log\\PIOViewer\\LPMC\\~$%s_%04dy%02dm%02dd.log", 															
				szFileAppName,
				strtSysTime.wYear,
				strtSysTime.wMonth, 
				strtSysTime.wDay);
		...
		{
//			sprintf( szFileName, "D:\\Bluetain_CTC\\_Log\\PIOViewer\\~$%s_%04dy%02dm%02dd.log", 															
			sprintf( szFileName, "_Log\\PIOViewer\\~$%s_%04dy%02dm%02dd.log", 															
				szFileAppName,
				strtSysTime.wYear,
				strtSysTime.wMonth, 
				strtSysTime.wDay);
...
*/		
		...
		{
			fprintf(fpText,"%s; %s, %s\n", szTimeStamp, pszAppName, pszErrorText);
			fflush(fpText);
		}
		if( NULL != fpText)
		{
