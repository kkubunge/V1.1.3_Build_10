...
static void LOG(const char * pszAppName, ELogType eLogOption, const char * pFormat, ...)
{
	// 0. declaration
	va_list args;		
	char szFileName[256];
	...
	sprintf( szTimeStamp, "%02d	%d:%02d:%02d",	strtSysTime.wDay, strtSysTime.wHour,
														strtSysTime.wMinute,
														strtSysTime.wSecond);
	...
	if( ELT_File == eLogOption || ELT_FileConsole == eLogOption)
	{
		...
		sprintf( szFileName, "f:\\datalog\\_RecipeHistory\\RH%04d-%02d.log",//%02d%02d%02d%02d.log", 															
//															szFileAppName,
															strtSysTime.wYear,
															strtSysTime.wMonth);//, 
//															strtSysTime.wDay);
