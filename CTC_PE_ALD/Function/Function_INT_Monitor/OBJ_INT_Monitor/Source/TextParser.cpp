...
CTextParser::CTextParser(const char *pText)
{
	...
	if(m_nSize > DEFAULT_BUFFER_SIZE-1) m_pText = new char[m_nSize+1];
...
int CTextParser::GetWords(char ppBuf[][80], int nPieces, bool bMakeUpper, char chToken)
{
	int i;
	for(i=0; i<nPieces; i++)
	{
...
bool CTextParser::GetString(char *pBuf, char chToken)
{
	...
	*pBuf = 0;
	...
	int i;
	for(i=0; i<MAX_BUF && PeekChar() != chToken
		&& PeekChar() != '\n' && (! IsEnd()); i++)
	{
		szBuf[i] = GetChar();
		if('\r' == szBuf[i] && '\n' == PeekChar())
...
bool CTextParser::GetBool(bool *pValue, const char *szTrue, const char *szFalse, char chToken)
{
	bool bRet = false;
	char szBuf[MAX_BUF] = "";
	char szTrue_A[MAX_BUF] = "";
	char szFalse_A[MAX_BUF] = "";
	if(GetWord(szBuf, chToken) == true)
	{
...
{
	char szBuf_A[MAX_BUF];
	char szBuf_B[MAX_BUF];
...
bool CTextParser::IsNumeric(const char *szStr)				// 2008.12.17
{
	if(*szStr >= '0' && *szStr <= '9') return true;
	return false;
}