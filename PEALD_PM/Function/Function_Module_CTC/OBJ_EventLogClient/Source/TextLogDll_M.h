...
// Log File Name
#define LOG_FILE_PATH				"_Log\\EventLog\\EventLogCli.log"
...
extern void __LOG(const char *szLog)			{ _gLogType2(szLog, 0); }		// 2013.06.10
//.......
//.......
//--------------------------------------------------------------------
...
static BOOL _gInitLogDll(const char *szLogFile, const char *szCfgFile, BOOL bConfigLog)
{
	...
	do {
		...
		if(NULL != g_hLogDllModule)
		{
			...
			// 2011.07.27
			// sprintf(szTemp, "SET_FILE_CR_P_HOURS %d %d", g_nInstanceID, 1);
			// _gTextLogDll_Command(szTemp, NULL);	
			// sprintf(szTemp, "SET_KEEP_DAYS %d %d", g_nInstanceID, 10);
			// _gTextLogDll_Command(szTemp, NULL);
			// 2013.06.13
			// sprintf(szTemp, "SET_LOG_TYPE %d %d", g_nInstanceID, LOG_TYPE1);
			// _gTextLogDll_Command(szTemp, NULL);
		}
		else
		{
