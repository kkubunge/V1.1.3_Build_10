...
long CTimer::GetProgressTime(int nType)
{
	...
	switch(nType)
	{
	case UNIT_MILLISECOND: return m_lProgress;
	case UNIT_SECOND: return (m_lProgress / 1000);
	case UNIT_MINUTE: return (m_lProgress / (1000 * 60));
	case UNIT_HOUR: return (m_lProgress / (1000 * 60 * 60));
	case UNIT_DAY: return (m_lProgress / (1000 * 60 * 60 * 24));
	}
	return 0;
}

double CTimer::GetProgressTime2(int nType)
{
	double dbTime = 0;
	if(TIMER_RUN == m_nTimerStatus) m_lProgress = labs(clock() - m_clkBegin);	// 2006.02.01
	GetLocalTime(&m_NowTime);
	switch(nType)
	{
	case UNIT_MILLISECOND: dbTime = m_lProgress;
	case UNIT_SECOND: dbTime = m_lProgress; dbTime /= 1000; break;
	case UNIT_MINUTE: dbTime = m_lProgress; dbTime /= (1000 * 60); break;
	case UNIT_HOUR: dbTime = m_lProgress; dbTime /= (1000 * 60 * 60); break;
	case UNIT_DAY: dbTime = m_lProgress; dbTime /= (1000 * 60 * 60 * 24); break;
	}
	return dbTime;
}
