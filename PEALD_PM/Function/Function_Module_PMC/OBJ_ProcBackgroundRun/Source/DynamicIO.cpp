...
int CDynamicIO::RegistIOChannel(char *szIOName, int nIOType)	// 2006.03.30
{
	int nIoIdx = -1;
	int i;
...
void CDynamicIO::dWRITE_DIGITAL(int nIoIndex, int nValue, BOOL *pResult)
{
	...
	if(m_pIOList[nIoIndex].nIOPoint == -1 || m_pIOList[nIoIndex].nIOType != _K_D_IO)
	{
		#ifdef _DEBUG
...
{
	if(m_pIOList == NULL) { printf("-----> IO List is not initialized !\n"); *pResult = FALSE; return; }
	if(m_pIOList[nIoIndex].nIOPoint == -1 || m_pIOList[nIoIndex].nIOType != _K_A_IO)
	{
		#ifdef _DEBUG
...
{
	if(m_pIOList == NULL) { printf("-----> IO List is not initialized !\n"); *pResult = FALSE; return; }
	if(m_pIOList[nIoIndex].nIOPoint == -1 || m_pIOList[nIoIndex].nIOType != _K_S_IO)
	{
		#ifdef _DEBUG
...
int CDynamicIO::dRUN_FUNCTION(int nIoIndex, char *szParam)
{
	...
	if(m_pIOList == NULL) { printf("-----> IO List is not initialized !\n"); return nRet; }
	if(m_pIOList[nIoIndex].nIOPoint == -1 || m_pIOList[nIoIndex].nIOType != _K_F_IO)
	{
		#ifdef _DEBUG
