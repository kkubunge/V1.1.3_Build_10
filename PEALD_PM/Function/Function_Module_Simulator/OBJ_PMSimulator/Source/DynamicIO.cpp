...
void CDynamicIO::dWRITE_DIGITAL(int nIoIndex, int nValue, BOOL *pResult)
{
	...
	{
		#ifdef _DEBUG
		printf("--PLCSim---> Invalid IO [Name:%s,Type:%d,Point:%d,nValue:%d] in dWRITE_DIGITAL\n",
				m_pIOList[nIoIndex].szIOName, m_pIOList[nIoIndex].nIOType, m_pIOList[nIoIndex].nIOPoint,nValue);
		#endif
		*pResult = FALSE;
		return;
	}
	...
	{
		printf("--PLCSim---> Invalid IO Type[Name:%s,Type:%d,Point:%d] in dWRITE_DIGITAL\n",
