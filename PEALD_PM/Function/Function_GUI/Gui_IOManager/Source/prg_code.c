...
IO_Name_String_Map IO_STR_Map_Table[] = {
	{ "IOLockDO"			,	_K_D_IO	,	IOLockDO,			0	} ,	// 2015.01.27 IO Lock Mode Added by hyuk 
	{ "IO_Driver_Loding"	,	_K_D_IO	,	IO_Driver_Loding,	0	} ,	// 2019.11.04 SIMULATION_MODE IO Value Change Use
...
char  STRING_DO[12]  = "DO";
char  STRING_DIO[12] = "DIO";
char  STRING_XO[12]  = "XO";
char  STRING_AO[12]  = "AO";
char  STRING_AIO[12] = "AIO";
char  STRING_SO[12]  = "SO";

// 2015.01.27 IO Lock Mode Added by hyuk <<
enum { LOCK, UNLOCK };
int LOCK_CONTROL = 0;
...
//
//
BOOL Check_IOType(int Find, char *Buffer, int nIOType)
{
	...
	// 2015.01.27 IO Lock Mode Added by hyuk <<
	if(UNLOCK == READ_DIGITAL(IOLockDO, &nCommStatus) || LOCK_CONTROL == UNLOCK) return TRUE;
	// 2015.01.27 IO Lock Mode Added by hyuk >>
	
	nLenth = strlen(Buffer);	

	if(nLenth > 3)
	{
		memset(nTemp,  0, sizeof(nTemp));
		memset(nTemp2, 0, sizeof(nTemp2));
		
		memset(szExcept,  0, sizeof(szExcept));
		memset(szExcept2, 0, sizeof(szExcept2));

		strcpy(nTemp , Buffer+nLenth-2);
		strcpy(nTemp2, Buffer+nLenth-3);

		sprintf(szExcept,  "M651_Gain");
		sprintf(szExcept2, "M651_Phase");

		// 2015.01.10 Exceptional IO Check by hyuk
		switch(nIOType)
		{
			case 0:	//Digital
				break;
			...
			case 1: //Analog
				if(NULL != strstr(Buffer, szExcept) || NULL != strstr(Buffer, szExcept2))
				{
					return TRUE;
				}	
				break;
		...
		case 0:	//Digital
			if(!stricmp(nTemp, STRING_DO) || !stricmp(nTemp, STRING_XO) || !stricmp(nTemp2, STRING_DIO))
			{
				bRet = TRUE;
			}
			break;
		...
		case 1: //Analog
			if(!stricmp(nTemp, STRING_AO) || !stricmp(nTemp2, STRING_AIO))
			{
				bRet = TRUE;
			}	
			break;
		case 2: //String
			if(!stricmp(nTemp, STRING_SO))
			{
				bRet = TRUE;
			}
			break;
	...
	for ( i = DISPLAY_DIGITAL_START_POINTER , j = 0 ; ( k < DIGITAL_TOTAL_COUNT() ) && ( i < DIGITAL_SELECT_COUNT() ) && ( j < USE_DISPLAY_COUNT ) && ( ConsoleGuiForPres_USE == 1 ); k++ ) { // 2006.05.29
		...
		if ( D_IO_SELECT( k ) ) {
			...
			if ( AllMode ) {
				...
				// 2014.11.14 Real/Virtual IO Separation by hyuk
				//_dDIGITAL_NAME(k, Buffer2);
				bIOResult = Check_IOType(k, D_IO_NAME(k), 0);

				if(bIOResult)
				{
					wsprintf( buffer, "%4d" , i+1 );
					KGUI_DRAW_StrBox22( hDC , DISPLAY_START_X_FOR_DIGITAL , DISPLAY_START_Y_FOR_DIGITAL + ( j * DISPLAY_SIZE_Y_FOR_DIGITAL ) , buffer , DISPLAY_SIZE_X_FOR_DIGITAL , 1 , COLOR_White , COLOR_DarkGray , COLOR_Black , 13 );	//Number
					if ( ConsoleGuiForPres_USE != 1 ) return;
					KGUI_DRAW_StrBox22( hDC , DISPLAY_START_X_FOR_DIGITAL1 , DISPLAY_START_Y_FOR_DIGITAL + ( j * DISPLAY_SIZE_Y_FOR_DIGITAL ) , D_IO_NAME(k) , DISPLAY_SIZE_X_FOR_DIGITAL1 , 1 , COLOR_DarkGray , COLOR_White , COLOR_Black , 13 );	//Output
					DIGITAL_DATA_BUFFER_WRITE[ j ] = DIGITAL_DATA_BUFFER_READ[ j ] = -1;
				}
				else
				{
					wsprintf( buffer, "%4d" , i+1 );
					KGUI_DRAW_StrBox22( hDC , DISPLAY_START_X_FOR_DIGITAL , DISPLAY_START_Y_FOR_DIGITAL + ( j * DISPLAY_SIZE_Y_FOR_DIGITAL ) , buffer , DISPLAY_SIZE_X_FOR_DIGITAL , 1 , COLOR_White , COLOR_DarkGray , COLOR_RightRed , 13 );	//Number
					if ( ConsoleGuiForPres_USE != 1 ) return;
					KGUI_DRAW_StrBox22( hDC , DISPLAY_START_X_FOR_DIGITAL1 , DISPLAY_START_Y_FOR_DIGITAL + ( j * DISPLAY_SIZE_Y_FOR_DIGITAL ) , D_IO_NAME(k) , DISPLAY_SIZE_X_FOR_DIGITAL1 , 1 , COLOR_DarkGray , COLOR_White , COLOR_RightRed , 13 );	//Output
					DIGITAL_DATA_BUFFER_WRITE[ j ] = DIGITAL_DATA_BUFFER_READ[ j ] = -1;
				}
			}
			if ( ConsoleGuiForPres_USE != 1 ) return;
	...
	for ( i = DISPLAY_ANALOG_START_POINTER , j = 0 ; ( k < ANALOG_TOTAL_COUNT() ) && ( i < ANALOG_SELECT_COUNT() ) && ( j < USE_DISPLAY_COUNT ) && ( ConsoleGuiForPres_USE == 2 ); k++ ) {
		if ( A_IO_SELECT( k ) ) {
			...
			if ( AllMode ) {
				...
				// 2014.11.14 Real/Virtual IO Separation by hyuk
				//_dANALOG_NAME(k, Buffer2);
				bIOResult = Check_IOType(k, A_IO_NAME(k), 1);
				...
				{
					wsprintf( buffer , "%4d" , i+1 );
					KGUI_DRAW_StrBox22( hDC , DISPLAY_START_X_FOR_ANALOG  , DISPLAY_START_Y_FOR_ANALOG + ( j * DISPLAY_SIZE_Y_FOR_ANALOG ) , buffer       , DISPLAY_SIZE_X_FOR_ANALOG  , 1 , COLOR_White , COLOR_DarkGray , COLOR_RightRed , 13 );
					if ( ConsoleGuiForPres_USE != 2 ) return;
					KGUI_DRAW_StrBox22( hDC , DISPLAY_START_X_FOR_ANALOG1 , DISPLAY_START_Y_FOR_ANALOG + ( j * DISPLAY_SIZE_Y_FOR_ANALOG ) , A_IO_NAME(k) , DISPLAY_SIZE_X_FOR_ANALOG1 , 1 , COLOR_DarkGray , COLOR_White , COLOR_RightRed , 13 );
					ANALOG_DATA_BUFFER_WRITE[ j ] = ANALOG_DATA_BUFFER_READ[ j ] = -1;
				}
			}
			if ( ConsoleGuiForPres_USE != 2 ) return;
	...
	for ( i = DISPLAY_STRING_START_POINTER , j = 0 ; ( k < STRING_TOTAL_COUNT() ) && ( i < STRING_SELECT_COUNT() ) && ( j < USE_DISPLAY_COUNT ) && ( ConsoleGuiForPres_USE == 3 ); k++ ) {
		if ( S_IO_SELECT( k ) ) {
			...
			if ( AllMode ) {
				...
				// 2014.11.14 Real/Virtual IO Separation by hyuk
				//_dSTRING_NAME(k, Buffer2);
				bIOResult = Check_IOType(k, S_IO_NAME(k), 2);
				...
				{
					wsprintf( buffer , "%4d" , i+1 );
					KGUI_DRAW_StrBox22( hDC , DISPLAY_START_X_FOR_STRING  , DISPLAY_START_Y_FOR_STRING + ( j * DISPLAY_SIZE_Y_FOR_STRING ) , buffer       ,  DISPLAY_SIZE_X_FOR_STRING  , 1 , COLOR_White , COLOR_DarkGray , COLOR_RightRed , 13 );
					if ( ConsoleGuiForPres_USE != 3 ) return;
					KGUI_DRAW_StrBox22( hDC , DISPLAY_START_X_FOR_STRING1 , DISPLAY_START_Y_FOR_STRING + ( j * DISPLAY_SIZE_Y_FOR_STRING ) , S_IO_NAME(k) ,  DISPLAY_SIZE_X_FOR_STRING1 , 1 , COLOR_DarkGray , COLOR_White , COLOR_RightRed , 13 );
				}
...
//	char cMessage[256];
	...
	sprintf(cCheckPM,"CTC_CommStatus");  //AKRA, Bluetain, DB21 (구)아이피에스 설비 PM Commstatus I/O
//	sprintf(cCheckPM,"PM_COMMSTATUS");   //HP1/2, MAHA, SALA, SLSI, Tandem (구)아토 설비 PM Commstatus I/O

	
	// 2019. 11. 04 gjyu - SIMULATION_MODE IO Value Change Use
...
BOOL APIENTRY Console_Gui_For_MainIO_Proc( HWND hdlg , UINT msg , WPARAM wParam , LPARAM lParam ) {
	...
	switch ( msg ) {
		...
		case WM_INITDIALOG:
			...
			if ( LOCK_USE )
			{
				KWIN_Item_Enable( hdlg , IDC_BUTTON_LOCK );
				g_hdlg = hdlg;
			}
			else
			{
				KWIN_Item_Hide( hdlg , IDC_BUTTON_LOCK );
			}
			// 2015.01.27 IO Lock Mode Added by hyuk >>
...
//					if ( D_IO_TYPE( Find ) == OUTPUT_IO_TYPE ) { // 2003.08.26
					if ( D_IO_TYPE( Find ) >= OUTPUT_IO_TYPE ) { // 2003.08.26
						...
						bRes = Check_CommStatus(D_IO_NAME(Find));
						
						// 2014.11.14 Real/Virtual IO Separation by hyuk
						bIOResult = Check_IOType(Find, D_IO_NAME(Find), 0);

						//if ( EnumCnt <= 0 && !bRes && bIOResult ) {
						if ( EnumCnt <= 0 ) {
...
//					if ( A_IO_TYPE( Find ) == OUTPUT_IO_TYPE ) { // 2003.08.26
					if ( A_IO_TYPE( Find ) >= OUTPUT_IO_TYPE ) { // 2003.08.26
						...
						bRes = Check_CommStatus(A_IO_NAME(Find));

						// 2014.11.14 Real/Virtual IO Separation by hyuk
						bIOResult = Check_IOType(Find, A_IO_NAME(Find), 1);
...
//				for ( j = 0 ; j < DISPLAY_COUNT_FOR_STRING ; j++ ) {
				...
				if ( iFind >= 0 && iFind < STRING_SELECT_COUNT() ) {
					...
					bRes = Check_CommStatus(S_IO_NAME(Find));
					//bRes = Check_CommStatus(Buffer);

					// 2014.11.14 Real/Virtual IO Separation by hyuk
					bIOResult = Check_IOType(Find, S_IO_NAME(Find), 2);
		...
		case WM_COMMAND:
            switch ( LOWORD( wParam ) ) {
				...
				case IDC_BUTTON_LOCK :
					if( LOCK_USE == TRUE )
					{
						if ( LOCK_CONTROL == 0 )	LOCK_CONTROL = 1;
						else						LOCK_CONTROL = 0;
						
						switch( LOCK_CONTROL ){
						case 0:
							KWIN_Item_String_Display( hdlg , IDC_BUTTON_LOCK , "IO LOCK" );
							break;
						case 1 :
							KWIN_Item_String_Display( hdlg , IDC_BUTTON_LOCK , "IO UNLOCK" );
							break;
						}
						return TRUE;
					}
...
void LOCK_Thread()
{
	...
	while(TRUE) 
	{
		if( UNLOCK == LOCK_CONTROL || UNLOCK == READ_DIGITAL(IOLockDO, &CommStatus) )
		{
			nCount++;
		}
		else
		...
		if(nCount > 180)
		{
			WRITE_DIGITAL(IOLockDO, LOCK, &CommStatus);
			LOCK_CONTROL = 0;
			KWIN_Item_String_Display( g_hdlg , IDC_BUTTON_LOCK , "IO LOCK" );
		}
...
//	REGIST_FILE_INFO( "Gui IO Manager 3.53(2005.02.28)" ); // CPU OverHead Bug Update
	...
	for ( i = 0 ; i < (signed) strlen( NAME1 ) ; i++ ) {
		...
		if ( tolower(NAME1[i]) == 'l' ) LOCK_USE = TRUE;
	}
	//
	//>>-------------------------------------------------------------------------
	...
	// 2015.01.27 IO Lock Mode Added by hyuk <<
	_beginthread((_THREAD_FUNC) LOCK_Thread, 0, NULL);
	// 2015.01.27 IO Lock Mode Added by hyuk >>
