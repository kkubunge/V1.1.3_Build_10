...
//... 2016.02.25  OnOff&direct Mode Control 
#define HT_CMD_MANUAL_DR	0x07
#define HT_CMD_AUTO_DR		0x03
#define HT_CMD_TUNE_DR		0x0B
...
void UpdateCtrlMd(int nCmd, char *pBuf)
{
	...
	for(i=0; i<MAX_CHANNEL; i++)
	{
		...
		byData = *(pData+1);
        //... 2016.02.25
		if ((byData&0x01)==0x01) Ctrl = DZC;
		else if((byData&0x03)==0x03) Ctrl = ONOFF;
		else Ctrl=TP;
...
BOOL SetCtrlMd(int nHtrNum, int nData)
{
	...
	else if ( nData == 6 ) usData = HT_CMD_MANUAL_DR;		// Manual ( OnOff&Direct Mode )
	else if ( nData == 7 ) usData = HT_CMD_AUTO_DR;			// Auto
	else if ( nData == 8 ) usData = HT_CMD_TUNE_DR;			// Tune	
	else return FALSE;
...
BOOL SetOnOff(int nHtrNum, int nData)
{
	...
	case 6: usData = HT_CMD_MANUAL_DR;	break;		// Manual ( OnOff&Direct Mode )
	case 7: usData = HT_CMD_AUTO_DR;	break;		// Auto
	case 8: usData = HT_CMD_TUNE_DR;	break;		// Tune	
    //... 
	}
